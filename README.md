...
Current package slice: `PulseTrade.Comm.Spa.Dynamic 0.1.3-beta73`, paired with `PulseTrade.Comm.Spa 0.2.5-beta80`. The current Actor Argu Add Target Key schema is `[proxyActorAddress; "target-v1"; targetActorAddress; duTypeOrTemplateKey; canonicalArgString]`; the invalidated beta64 `actor-argu-proxy` hidden rewrite path is historical only. Add Actor Key keeps the native actor address as its key and relies on the PTCS fabric's installed platform provider for low-cognitive `string -> fCell2<string> -> native actor -> fCell2<string> -> reply` dispatch. Beta73 retains beta72 bundle discovery and inbound Canvas classification behavior; this release only aligns the exact PTCS dependency for the durable submission policy seam.

Realtime TA新路徑使用`PulseTrade.Comm.Spa.Dynamic.Ptcs 0.1.0-alpha3` server adapter與`PulseTrade.Comm.Spa.Dynamic.Ptcs.Client 0.1.0-alpha2` pure WebSharper client adapter。server端exact PTCS beta82 + Contracts alpha4，持有authenticated transient context與canonical reducer；browser只走same-origin `/sync/ws`、bounded `ta-browser.v1` wire與Renderer，不接受任意URL或credential。Legacy Dynamic bundle暫不reference新adapters。WebSharper 10.1.5的stale `wsfscservice`可能讓後續compile無診斷退出；先停止stale helper再重建，禁止以縮短project path或關閉compiler掩蓋。禁止raw JavaScript或HTTP polling。
