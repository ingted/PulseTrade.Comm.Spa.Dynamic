...
Current package slice: `PulseTrade.Comm.Spa.Dynamic 0.1.3-beta41`, paired with `PulseTrade.Comm.Spa 0.2.5-beta51`. `src\poc.full.nuget.journal.ACL.fsx -- --no-wait` verifies the ACL/Login dual-auth path: 81-style GitHub OAuth host, 82-style PTCS.Login local host, DamnWZ/AssTerry pages, Echo/PingPong target keys, ACL snapshot, Dynamic bundle, durable ActorArgu echo, PingPong stop filtering, and fixed actor name reuse after stop. Playwright MCP verified the PTCS.Login page, admin `+ Page`, Terry黑粉 no `+ Page` / no Add target, FormInput visible, and no `ptcs.extension.post` alert.

`0.1.3-beta41` is packaged with `WebSharperRunCompiler=false` because WebSharper `10.1.5.674` currently crashes `wsfsc.exe` when compiling Dynamic against PTCS `0.2.5-beta51`; the project still type-checks and the package includes the existing verified `src/wwwroot/js` Dynamic bundle. The compiler crash is tracked as a follow-up; do not treat it as an ACL/Login runtime failure.
