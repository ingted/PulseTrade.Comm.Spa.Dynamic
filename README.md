...
The current renderer already builds a page-level Actors UI with action shell, count cards, host/port node blocks, hierarchy rows, grid rows, and boxed `+` / `-` tree toggles. A PTCS source-host Playwright MCP gate verified that Dynamic owns the `/actors` DOM when loaded: fallback rows are `0`, blocks are ordered PTCS Host -> GW Host -> RN Host -> Unknown, full `akka.tcp://...` addresses are visible, and tree toggle clicks actually collapse and expand rows. The `0.1.3-beta23` package rollout was deployed to public 81 with PTCS `0.2.5-beta39`; the public `/actors` page renders `Actors / Dynamic` instead of the beta38 Canvas-summary/fallback split view, and a Playwright MCP screenshot confirms the boxed collapse state. NuGet flat-container indexing for `0.1.3-beta23` is confirmed. The reusable accepted-path gate is `G:\PulseTrade2.fs\Libs\PulseTrade.Comm.Spa\Scripts\verify.actorsPageDynamic.playwright.fsx`. Full strict schema parsing, production report actions, restart/failover visual states, and Dynamic absent/unsupported renderer failure-injection remain tracked by `DYN-WBS-519`.
