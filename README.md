...
Realtime TA新路徑使用獨立`PulseTrade.Comm.Spa.Dynamic.Ptcs 0.1.0-alpha2` server adapter，exact PTCS beta82 + Contracts alpha4。它把authenticated transient context與typed client frame交給host backend，使用canonical reducer後回full runtime state。Legacy Dynamic bundle暫不reference該package；WebSharper 10.1.5會在recursive browser wire/dependency metadata merge時無診斷結束。Browser adapter改用下一版bounded non-recursive TA wire，禁止raw JavaScript或HTTP polling。
