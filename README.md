...
Current package slice: `PulseTrade.Comm.Spa.Dynamic 0.1.3-beta38`, paired with `PulseTrade.Comm.Spa 0.2.5-beta48`. `src\poc.full.nuget.journal.fsx -- --no-wait` verifies the ActorRegistry PingPong stop/reload path: the PingPong actor is projected as `terminated`, default active ActorTree DSL filters it, and `includeOffline` keeps it only for diagnostics. The same POC also exposes `ensureEchoActorRegistered()`, `stopEchoActor()`, and `recreateEchoActor()`; do not rerun `ActorOfRegistered(..., actorName)` with the fixed `nuget-journal-echo` name in the same ActorSystem because Akka actor names are unique under `/user`. Public 81 deployment is `live81-ptcs-beta48-dynamic-beta38-pingpong-registry-20260629161443`.
