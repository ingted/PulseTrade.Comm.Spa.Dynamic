...
let defaultArgumentsText =
    ...
    $"""--host 0.0.0.0 --local-port 82 --site-sharing isolated --pcsl-root "{pathArg defaultPcslRoot}" --delivery-profile nuget-journal-acl-live --actor-name nuget-journal-acl-echo --cluster-port {defaultClusterPort} """
...
let parser =
    ArgumentParser.Create<CliArgs>(programName = "full.nuget.journal.ACL2.NoGithubOAuth.fsx")
...
printfn
    "startup preflight ok: local=%s:%d cluster=127.0.0.1:%d dynamic=%b"
    host
    localPort
    clusterPort
    ifDynaPort
...
let actorRegistrySettings =
    ...
    |> ActorRegistrySettings.withMetadata (
        Map.ofList
            [ "ptcs.dynamic.poc", "full.nuget.journal.ACL2.NoGithubOAuth.fsx"
...
let stopPocFullNugetJournalAclHosts () =
    if pocFullNugetJournalStopped then
        printfn "full.nuget.journal.ACL2.NoGithubOAuth host already stopped."
    else
        ...
        printfn "full.nuget.journal.ACL2.NoGithubOAuth host stopped."
...
try
            ...
            Some(
                ActorArgu.sendDurableAsync
                    ...
                    { Send =
                      ...
                      IdempotencyKey = None
                      Source = Some "full.nuget.journal.ACL2.NoGithubOAuth.fsx"
                      DeadlineAtUtc = None }
    ...
    printfn "Ports             local=%d cluster=%d dynamic=%b" localPort clusterPort ifDynaPort
