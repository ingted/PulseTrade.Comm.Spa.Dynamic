...
- current public release：`PulseTrade.Comm.Spa.Dynamic.Renderer 0.1.76`，exact依賴Contracts `[0.1.29]`與FSharp.Core `[10.1.400]`。K-bar row的Y-domain只取current viewport projected candles與same-row projected lines，並把自SVG edge起算的上下15 CSS px反算成viewBox/data padding，row resize不放大留白。所有chart row的authored/default plot height封頂250 CSS px，但manual candle/scalar resize仍可到720/480px；reset/reload回capped default。一般line／SMA寬度夾在1–2 CSS px並使用`non-scaling-stroke`；overview左右可見邊界與selection依RFC-0028。Renderer保留Backtest Presentation UX的same-topology navigator refresh，並以document/data/transport diagnostics區分render原因；chunked transport只在完整candidate commit後進入Renderer。Marker使用bounded SVG overlay、document trace order → bucket order的跨trace deterministic shape lanes；`TriangleUp／TriangleDown`方向不受anchor改寫，`Outline`使用`fill="none"`且保留pointer hit target。marker不進數值圖例、Y-domain或額外time slot；直接marker hit以accepted exact slot更新shared cursor，一般plot仍依axis snap。row-local cursor timestamp使用SVG外固定32px gutter；line reader與大型candle projection維持bounded indexed/single-pass路徑，同topology patch不重掛chart stack。
