...
- current exact package：`PulseTrade.Comm.Spa.Dynamic.Renderer 0.1.7`，exact依賴Contracts `[0.1.5]`與FSharp.Core `[10.1.400]`。TA row顯示名稱依序採 authored `Options["label"]`、trace labels、typed row kind；row card與toolbar不得各自產生不同名稱。
