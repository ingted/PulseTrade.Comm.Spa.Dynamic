...
- current exact package：`PulseTrade.Comm.Spa.Dynamic.Renderer 0.1.0-alpha24`。`Document=None` 依 runtime poll/error狀態呈現 preparing、connecting、recovering、closed 或 unavailable；正常 bootstrap 不再被誤標成 terminal error。Current model gate 19/19，isolated F# Playwright使用2000-point fixture通過dual handles、drag-release commit與desktop/mobile geometry。
