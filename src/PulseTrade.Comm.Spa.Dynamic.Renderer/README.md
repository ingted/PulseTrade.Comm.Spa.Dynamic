...
- current exact package：`PulseTrade.Comm.Spa.Dynamic.Renderer 0.1.29`，exact依賴Contracts `[0.1.12]`與FSharp.Core `[10.1.400]`。Marker使用bounded SVG overlay、deterministic lanes與edge clipping；只呈現可視position，不進數值圖例或Y-domain。TA row顯示名稱依序採 authored `Options["label"]`、trace labels、typed row kind；row card與toolbar不得各自產生不同名稱。document shell cache key與action routing都使用current `DocumentId`／`CanvasInstanceId`。高尺度source candle只在presentation依actual base-axis slots展開成等值candles，保留canonical source interval且不複製storage series。shared temporal axis與series按`dataRef`、shared prefix與changed axis position增量prepare；visible projection採sorted-axis range lookup及reverse-assignment/path-compression，重疊causal suffix不會展開成二次方工作。mousemove經單一requestAnimationFrame直接更新固定crosshair/value DOM，不進入chart-wide View；同一event-time preview只發送實際變更的candle/trace與固定高度row-value band，不重建chart stack。reader以mutable cell保留最新geometry，避免WebSharper將reader eta-reduce成initial snapshot；只有timestamp topology或document/view topology改變才重建main chart。
