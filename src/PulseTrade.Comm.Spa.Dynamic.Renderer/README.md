...
RFC-0028 local candidate為Renderer `0.1.75`、Contracts `[0.1.29]`、Interactive.Client `0.1.66`；`0.1.74/0.1.65`因真SPAA coarse-row marker hover反證已退休。它包含RFC-0027 typed dark plot／Histogram polarity，並以`DefaultView.visibleBars`初始化fresh canvas；plot marker只保留glyph/tooltip，每列在cursor gutter與plot間提供固定24px OFI band，最多4筆＋`+N`。Marker glyph直接選accepted placement slot，OFI item保留原始event time；一般plot hover仍依row axis。Overview selection為淺灰，visual boundaries為亮綠2 CSS px且不改transparent hit targets。此graph尚待Daedalus真SPAA GREEN，未取代上述public release。
