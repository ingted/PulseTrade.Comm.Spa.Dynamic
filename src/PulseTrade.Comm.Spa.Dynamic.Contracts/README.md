...
Current exact package：`PulseTrade.Comm.Spa.Dynamic.Contracts 0.1.5`，exact依賴FSharp.Core `[10.1.400]`；current contract gate 21/21。Typed與decoded frame均執行相同validation；single patch最多64 operations，另受500 items與16MiB frame限制。`RuntimeCache`只接受reducer已確認的bounded projection；generic identity為owner提供的`OwnerFingerprint`與Dynamic schema revision。`RuntimeCacheEntryValidation`是WebSharper browser與server共用的完整entry語意gate；JSON合法但Document/Snapshot不一致的entry同樣fail closed。SPAA可將canonical `QueryFingerprint`映射為OwnerFingerprint，但program/data-source/query細節不進generic contract，且browser cache不得取代authoritative host state。
