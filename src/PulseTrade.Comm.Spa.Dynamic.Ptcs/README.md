...
- Current exact package：`PulseTrade.Comm.Spa.Dynamic.Ptcs 0.1.50`；exact依賴PTCS `[0.2.46]`與Contracts `[0.1.29]`。adapter保留`ta-marker.v2` payload與structured rejection；chunked Snapshot transport、v1相容解碼、schema 3 cache及last-good規則由Contracts統一擁有，current wire gate須由exact-package suite驗證。
