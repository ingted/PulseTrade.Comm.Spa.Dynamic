...
Current exact package is `PulseTrade.Comm.Spa.Dynamic.Ptcs.Client 0.1.54`, consuming Contracts `[0.1.14]` and Renderer `[0.1.38]`。client保留marker trace與bucket，unknown trace kind/version fail closed，不可降級成candlestick；triangle direction、true hollow、aggregate lane與shared cursor interaction由同一Renderer contract處理。同一event-time的live preview只更新實際變動的SVG element/trace與row-value band，不重建chart stack；shared-axis indexed projection與單一rAF cursor DOM hot path由Renderer統一處理。Add/Remove/Edit Row、Change Query、shared cursor、visible range、full-data export與Reset Canvas都走同一one-in-flight typed action channel；PTCS host/provider必須回authoritative frame。
