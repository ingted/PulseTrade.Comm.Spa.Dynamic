...
Current exact package is `PulseTrade.Comm.Spa.Dynamic.Ptcs.Client 0.1.59`, consuming Contracts `[0.1.19]` and Renderer `[0.1.45]`。client保留marker trace與bucket，unknown trace kind/version fail closed，不可降級成candlestick；triangle direction、true hollow、shape/visible-label collision lanes與shared cursor interaction由同一Renderer contract處理。同一event-time的live preview只更新實際變動的SVG element/trace、row-local timestamp/value band及navigator shell signal，不重建chart stack；shared-axis indexed projection與單一rAF cursor DOM hot path由Renderer統一處理。Add/Remove/Edit Row、Change Query、shared cursor、visible range、full-data export與Reset Canvas都走同一one-in-flight typed action channel；PTCS host/provider必須回authoritative frame。`0.1.58`缺少same-topology OverviewStripe shell refresh，不得作SPAA final graph。
