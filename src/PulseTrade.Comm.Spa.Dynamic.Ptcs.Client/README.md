...
Current exact package is `PulseTrade.Comm.Spa.Dynamic.Ptcs.Client 0.1.77`, consuming PTCS `[0.2.46]`、Contracts `[0.1.29]` and Renderer `[0.1.76]`。client保留marker trace與bucket，unknown trace kind/version fail closed，不可降級成candlestick；triangle direction、true hollow、shape/visible-label collision lanes、shared cursor與RFC-0028 direct marker exact-slot由同一Renderer contract處理。chunked Snapshot只在完整candidate通過canonical reducer後送入同一Renderer contract。同一event-time的live preview只更新實際變動的SVG element/trace、row-local timestamp/value band及navigator shell signal，不重建chart stack；shared-axis indexed projection與單一rAF cursor DOM hot path由Renderer統一處理。Add/Remove/Edit Row、Change Query、shared cursor、visible range、full-data export與Reset Canvas都走同一one-in-flight typed action channel；PTCS host/provider必須回authoritative frame。
