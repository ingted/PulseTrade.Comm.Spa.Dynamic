...
Current exact package is `PulseTrade.Comm.Spa.Dynamic.Ptcs.Client 0.1.0-alpha7-win10`, consuming Renderer `[0.1.0-alpha9]`。Browser revision fields use JSON-safe numbers and are validated back to domain `int64` at the server boundary；dispose waits for the transient close response before closing its dedicated socket。Add/Remove Row、Change Query與Reset Canvas都走同一one-in-flight typed action channel。
