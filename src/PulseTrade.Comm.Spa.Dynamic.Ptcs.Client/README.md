...
client lifecycle保證one-in-flight action/poll、timeout/backoff、bounded reconnect、full snapshot resync與terminal dispose。Renderer可在query pending期間只保留最新queued query，但client transport仍一次只送一筆；server-side validation rejection是`CommandRejected`，不會關閉仍健康的WebSocket。client只使用same-origin WebSocket，不做HTTP polling。
...
Current exact package is `PulseTrade.Comm.Spa.Dynamic.Ptcs.Client 0.1.53`, consuming Contracts `[0.1.13]` and Renderer `[0.1.37]`。client保留marker trace與bucket，unknown trace kind/version fail closed，不可降級成candlestick；triangle direction、true hollow、aggregate lane與shared cursor interaction由同一Renderer contract處理。同一event-time的live preview只更新實際變動的SVG element/trace與row-value band，不重建chart stack；shared-axis incremental prepared projection、index cursor lookup與單一rAF cursor DOM hot path由Renderer統一處理。Browser revision fields use JSON-safe numbers and are validated back to domain `int64` at the server boundary；dispose waits for transient close response before closing its dedicated socket。Add/Remove/Edit Row、Change Query、shared cursor、visible range、full-data export與Reset Canvas都走同一one-in-flight typed action channel；accepted ChangeQuery的local viewport selection由Renderer處理，不發第二個remote range action。RFC-0020的boundary extension沿用`VisibleRangeChanged`，PTCS host/provider必須回authoritative frame；client只轉送typed action並套用canonical reducer。
