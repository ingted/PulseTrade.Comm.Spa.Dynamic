...
RFC-0028 local candidate為Interactive.Client `0.1.67`，exact依賴Contracts `[0.1.29]`與Renderer `[0.1.76]`。bundle包含typed dark plot／Histogram polarity、fresh `visibleBars` initial viewport、固定24px marker OFI band、direct marker exact-slot precedence及overview淺灰／亮綠2px presentation。consumer只需升Contracts與Interactive.Client direct references，不新增Renderer direct reference。真SPAA GREEN與public push前，current public release仍是上一段的`0.1.61`。
