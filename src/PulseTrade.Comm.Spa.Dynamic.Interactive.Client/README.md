...
Current exact package：`PulseTrade.Comm.Spa.Dynamic.Interactive.Client 0.1.24`，exact依賴Contracts `[0.1.12]`、Renderer `[0.1.29]`與FSharp.Core `[10.1.400]`；bundle manifest版本須與nuspec一致。runtime v2 marker frame沿用同一reducer/renderer，unknown kind與invalid candidate fail closed並保留last-good。同一event-time的live preview只更新實際變動的SVG element/trace與row-value band，不得重建chart stack；mousemove由Renderer的單一rAF固定DOM hot path處理，不向chart-wide View fan-out。Host需處理`SharedCursorChanged`與`VisibleRangeChanged`，accepted只解除pending，authoritative range/data仍由後續RuntimeFrame提交。Reconnect只有在authoritative Snapshot被接受後才重設backoff，單純WebSocket open不視為恢復成功。shared temporal axis使3,820 bars x 28 scalar series只傳一份時間metadata，client retention hard cap為4,000。`BrowserRuntimeCache`提供8-entry bounded IndexedDB store；marker document使用cache schema 2，舊schema必須miss/resync。`writeAcceptedState`只保存通過canonical contract gate且已移除OPEN_END preview的finalized-prefix，`tryRehydrate`以current session identity重建`PausedForResync` state。read會套用Contracts共用的完整semantic validator，malformed或Document/Snapshot不一致的record會回`Miss`並刪除。內容永遠是可丟棄的display-first last-good projection，cached revision不供delta續接，也不能取代host/provider authority。
