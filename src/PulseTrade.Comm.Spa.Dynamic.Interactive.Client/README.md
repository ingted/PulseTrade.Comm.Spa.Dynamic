...
Current exact package：`PulseTrade.Comm.Spa.Dynamic.Interactive.Client 0.1.0-alpha19`，exact依賴Contracts `[0.1.0-alpha19]`、Renderer `[0.1.0-alpha45]`與FSharp.Core `[10.1.400]`；bundle manifest版本須與nuspec一致。Host需處理`SharedCursorChanged`與`VisibleRangeChanged`，accepted只解除pending，authoritative range/data仍由後續RuntimeFrame提交。Reconnect只有在authoritative Snapshot被接受後才重設backoff，單純WebSocket open不視為恢復成功。
