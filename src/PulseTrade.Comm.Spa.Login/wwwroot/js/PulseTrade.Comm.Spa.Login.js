...
function Main(){
  ...
  return globalThis.console.log("PulseTrade.Comm.Spa.Login bundle loaded and registered login renderer");
}
function registerLoginRenderer(){
  if("PulseTradeRegisterLoginRenderer"in globalThis)globalThis.PulseTradeRegisterLoginRenderer("ptcs-login-page", 100, mountLogin);
}
function mountLogin(root, configJson){
  const config=configFromJson(configJson);
  root.setAttribute("data-ptcs-login-renderer", extensionId());
  if(!(doc().body==null))doc().body.setAttribute("data-ptcs-login-renderer", extensionId());
  else null;
  ...
  append(brand, [element("div", "brand-mark", "PT"), element("p", "brand-title", "PulseTrade Comm Spa"), element("p", "brand-subtitle", "\u672c\u9801\u7531 PulseTrade.Comm.Spa.Login extension bundle \u5448\u73fe\u3002\u767b\u5165\u6210\u529f\u5f8c\u7531 server \u8a2d\u5b9a HttpOnly session cookie\uff0c\u518d\u56de\u5230\u53d7\u4fdd\u8b77\u7684 PTCS \u9801\u9762\u3002")]);
  ...
  const submit=setTestId("ptcs-login-submit", button("primary", "\u767b\u5165\u4e26\u8fd4\u56de PTCS"));
  const setError=(text) => {
  ...
  const submitLogin=() => {
    ...
    if(isBlank(request.userName)||isBlank(request.password))setError("\u8acb\u8f38\u5165\u5e33\u865f\u8207\u5bc6\u78bc\u3002");
      ...
      postJson(config.submitPath, JSON.stringify(request), (responseBody) => {
        const target=textOr(config.returnUrl, decodeJson(responseBody).returnUrl);
...
function configFromJson(configJson){
  return isBlank(configJson)?defaultConfig():decodeJson(configJson);
}
...
function postJson(url, payloadJson, onOk, onError){
  ...
  (globalThis.fetch(url, {
    ...
    headers:headers,
    body:payloadJson
  }).then((response) => response.text().then((responseBody) => response.ok?onOk(isBlank(responseBody)?"{}":responseBody):onError(isBlank(responseBody)?"POST "+String(url)+" "+String(response.status):responseBody))))["catch"]((error) => onError(errorMessage(error)));
}
...
function currentServerRealityId(){
  ...
  if(node==null||isBlank_1(node.textContent))return"legacy";
    ...
    return textOr_1("legacy", json(node.textContent).serverRealityId);
  }
  catch(m){
    return"legacy";
  }
...
function getJson(url, onOk, onError){
  ...
  (globalThis.fetch(url, options).then((response) => response.text().then((body) => response.ok?onOk(json(isBlank_1(body)?"{}":body)):onError(isBlank_1(body)?"GET "+String(url)+" "+String(response.status):body))))["catch"]((error) => onError(errorMessage_1(error)));
}
...
function mountAppendPage(page, definition){
  ...
  const newKeyInput=setTestId_1("append-key-input", input_1(textOr_1("\"Aster\"", definition.keyPlaceholder)));
            ...
            if(o==null)return null;
              ...
              if((isBlank_1(filterText)||exists((key) => asText_1(key).toLowerCase().indexOf(filterText.toLowerCase())!=-1, arrayOrEmpty(_4)))&&!isLocallyHiddenKeyId(keyId)){
                ...
                if(m_2==null)updated=New_9(keyId, _4, _5, definition.setName, 0, 0n, 0n, asText_1(event.createdAtUtc), []);
                  ...
                  updated=New_9(existing.keyId, _4, textOr_1(existing.displayName, _5), definition.setName, existing.valueCount, existing.minSequence, existing.maxSequence, textOr_1(existing.updatedAtUtc, event.createdAtUtc), existing.values);
                }
                  ...
                  if(length(keys)>0){
                    ...
                    buckets=map((bucket_1) => {
                      if(sameText(bucket_1.keyId, keyId)){
                        ...
                        return New_9(bucket_1.keyId, keys, bucket_1.displayName, bucket_1.setName, _6, minSequence>0n?minSequence:bucket_1.minSequence, _7, textOr_1(bucket_1.updatedAtUtc, value.createdAtUtc), merged);
                      }
                      else return bucket_1;
                    }, buckets);
                    if(!matched){
  ...
  const subscribeKeyRegistry=() => {
    const streamPageId=textOr_1(definition.pageId, definition.tabId);
  ...
  rerenderAddKeyBuilder=() => {
    ...
    else newKeyInput.setAttribute("placeholder", textOr_1("\"Aster\"", definition.keyPlaceholder));
  ...
  const applySnapshot=(source, data) => {
    ...
    dynamicActorsPageAccepted?nodes.setAttribute("style", "display:none;"):(nodes.removeAttribute("style"),iter((node) => {
      ...
      let _1=(append_1(title_1, [element_1("label", "", "Node"), element_1("h2", "", asText_1(node.nodeId))]),append_1(blockHead, [title_1, element_1("div", "state", asText_1(node.status)+" / "+joinValues(node.roles))]),iter((actor) => {
        ...
        let _2=(card.appendChild(cardTitle(textOr_1(actor.actorId, actor.displayName), actor.actorId, actor.status, line)),card.appendChild(addressRow),iter((routee) => {
...
function mountChat(page){
  ...
  function renderParticipants(){
    ...
    iter((p_1) => {
      ...
      const name=textOr_1(p_1.participantId, p_1.displayName);
    ...
    if(current==null)_1="No participant selected";
      ...
      const p=current.$0;
      _1=textOr_1(p.participantId, p.displayName)+" ("+p.participantId+")";
    }
      ...
      const fetchThread=(useCursor) => {
        ...
        getJson(url, (data) => {
          ...
          readJson(cacheKey_1, (cached) => {
            ...
            const nextAfterMessageId=textOr_1(cursor, data.nextAfterMessageId);
    ...
    try {
      ...
      if(responseStatus=="ok"){
        if(responseType=="subscribe")setChatWsState("subscribed");
          ...
          setStatus(state, "Sent "+textOr_1("message", response.message==null?"":response.message.messageId)+" "+asText_1(response.deliveryHint));
        }
...
function mountLoginFallback(root){
  ...
  routes.setAttribute("aria-label", "Login context");
  append_1(routes, [routeItem_1("P", "Protected route", textOr_1("/actors", config.protectedRoute)), routeItem_1("S", "Session cookie", textOr_1("ptc_login_session", config.sessionCookieName)), routeItem_1("A", "ACL mode", textOr_1("enabled or authenticated-only", config.aclLabel))]);
  ...
  append_1(providerPill, [element_1("span", "dot", ""), doc_1().createTextNode(textOr_1("PTCS.Login", config.providerLabel))]);
  ...
  errorBox.setAttribute("role", "alert");
  const userName=setTestId_1("ptcs-login-username", setId_1("username", input_1("admin")));
  userName.setAttribute("name", "username");
  userName.setAttribute("type", "text");
  userName.setAttribute("autocomplete", "username");
  const password=setTestId_1("ptcs-login-password", setId_1("password", input_1("\u8f38\u5165\u5bc6\u78bc")));
  password.setAttribute("name", "password");
  password.setAttribute("type", "password");
  password.setAttribute("autocomplete", "current-password");
  ...
  append_1(inlineRow, [checkboxLabel, setHref_1("/login/help", element_1("a", "link", "\u9700\u8981\u5354\u52a9?"))]);
  const form=element_1("form", "", null);
  form.setAttribute("method", "post");
  form.setAttribute("action", config.submitPath);
  const submit=setTestId_1("ptcs-login-submit", button_1("", "\u767b\u5165\u4e26\u8fd4\u56de PTCS"));
  const setError=(text) => {
    errorBox.textContent=textOr_1("\u767b\u5165\u5931\u6557\u3002\u8acb\u78ba\u8a8d\u5e33\u865f\u6216\u5bc6\u78bc\u3002", text);
    errorBox.className="error-box visible";
  };
  const submitLogin=() => {
    ...
    if(isBlank_1(request.userName)||isBlank_1(request.password))setError("\u8acb\u8f38\u5165\u5e33\u865f\u8207\u5bc6\u78bc\u3002");
      ...
      postJson_1(config.submitPath, request, (reply) => {
        const target=textOr_1(config.returnUrl, reply.returnUrl);
        globalThis.location.assign(target);
      }, (error) => {
        submit.removeAttribute("disabled");
        submit.textContent="\u767b\u5165\u4e26\u8fd4\u56de PTCS";
  ...
  append_1(card, [statusRow, element_1("h1", "", textOr_1("\u767b\u5165 PTCS", config.title)), element_1("p", "lead", textOr_1("\u4f7f\u7528 host \u63d0\u4f9b\u7684\u5e33\u865f\u767b\u5165\u3002\u6b0a\u9650\u7531\u767b\u5165\u5f8c\u53d6\u5f97\u7684 principal \u8207 ACL policy \u6c7a\u5b9a\u3002", config.lead)), errorBox, form, element_1("p", "footer-note", "Browser flow \u61c9\u53ea\u56de HttpOnly cookie\uff1bheadless/API/WS \u624d\u4f7f\u7528 bearer token\u3002\u63d0\u4ea4\u7aef\u9ede\u793a\u610f\u70ba /login/api/submit\u3002")]);
...
function pageDefinitionFromWire(wire){
  if(wire==null||asText_1(wire.schema)!="ptc.comm.spa.append-page.definition.v1"||isBlank_1(wire.pageId))return null;
    ...
    return Some(New_3(pageId, textOr_1(pageId, wire.tabId), textOr_1("/page/"+pageId, wire.path), textOr_1(pageId, wire.title), textOr_1(pageId, wire.setName), textOr_1("raw", wire.shape), asText_1(wire.description), textOr_1("\"Aster\"", wire.keyPlaceholder), textOr_1("JSON value", wire.valuePlaceholder), asText_1(wire.defaultKey), arrayOrEmpty(wire.tags)));
  }
}
function hiddenPageFromWire(wire){
  if(wire==null||asText_1(wire.schema)!="ptc.comm.spa.append-page.hidden.v1"||isBlank_1(wire.pageId))return null;
    ...
    const pageId=asText_1(wire.pageId);
    return Some([pageId, textOr_1(pageId, wire.tabId)]);
  }
}
...
function pageTitle(page){
  return textOr_1(asText_1(page.pageId), asText_1(page.title));
}
function pageTypeLabel(page){
  ...
  if(isActorArguPage(page)){
    if(shapeText=="fcell-chat")return"Actor Argu";
      ...
      return m==null?"Actor Argu":textOr_1("Actor Argu", m.$0.label);
    }
  }
  else {
    ...
    if(m_1==null)return"Raw";
      ...
      const shape=m_1.$0;
      return textOr_1(normalizeShapeText(page.shape), shape.label);
    }
  }
}
...
function postJsonText(url, payloadJson, onOk, onError){
  ...
  options.method="POST";
  options.headers=headers;
  options.body=textOr_1("{}", payloadJson);
  (globalThis.fetch(url, options).then((response) => response.text().then((responseBody) => response.ok?onOk(responseBody):onError(isBlank_1(responseBody)?"POST "+String(url)+" "+String(response.status):responseBody))))["catch"]((error) => onError(errorMessage_1(error)));
}
  ...
  if(isActorArguPage(page)){
    if(shapeText=="fcell-chat")return"actor-argu";
      ...
      if(m==null)return"actor-argu";
        ...
        const shape=m.$0;
        return textOr_1(normalizeShapeText(page.shape), shape.className);
      }
    }
  }
  else {
    ...
    if(m_1==null)return"raw";
      ...
      const shape_1=m_1.$0;
      return textOr_1(normalizeShapeText(page.shape), shape_1.className);
    }
  }
}
function pageTypeBadge(page){
  ...
  if(isActorArguPage(page)){
    if(shapeText=="fcell-chat")return"aa";
      ...
      const m=findAppendPageShape(page.shape);
      return m==null?"aa":textOr_1("aa", m.$0.badge);
    }
  }
  else {
    const m_1=findAppendPageShape(page.shape);
    ...
    if(isBlank_1(pageIdText)){
      ...
      return candidatesLoaded&&candidatePageId.toLowerCase()==normalizedInput.toLowerCase()?onDone():(setStatus(status, "Checking history"),getJson("/pages/api/tab-candidates?pageId="+encodeURIComponent(normalizedInput), (reply) => {
        ...
        if(length(candidates)===0){
          ...
          iter((candidate) => {
            appendOption("reuse:"+asText_1(candidate.tabId), "Reuse "+textOr_1(asText_1(candidate.pageId), asText_1(candidate.tabId))+" ("+(candidate.visible?"visible":"hidden")+")", binding);
          }, candidates);
...
function readPendingRealitySplit(onRead){
  readAllPendingRaw((commands) => {
    ...
    onRead(filter((command) =>!(command==null)&&textOr_1("legacy", command.serverRealityId)==reality, commands), filter((command) =>!(command==null)&&textOr_1("legacy", command.serverRealityId)!=reality, commands));
  });
}
...
function readAllPending(onRead){
  readAllPendingRaw((commands) => {
    const reality=currentServerRealityId();
    onRead(filter((command) =>!(command==null)&&textOr_1("legacy", command.serverRealityId)==reality, commands));
  });
}
...
let _c_1=Lazy((_i) => class $StartupCode_Library {
  ...
  static extensionId;
  static {
    this.extensionId="pulse-trade-comm-spa-login";
  }
});
