...
function renderAddKey(ctx){
  ...
  if(!(shape=="actor-dynamic-target"||shape=="actor-argu-target"||shape=="actor-argu")&&!supportsProxy)return null;
    ...
    const defaultProxyAddress_1=o_1==null?"":o_1.$0;
    const defaultTargetAddress=isExplicitTargetKey(defaultKeyParts_1)&&length(defaultKeyParts_1)>2?get(defaultKeyParts_1, 2):"";
    const defaultTypeName=dynamicTypeNameFromKeyParts(defaultKeyParts_1);
    if(length(keys)===0)return Some(errorNode("No Dynamic Argu schemas are registered."));
      ...
      const proxyLabel_1=element("label", "dynamic-argu-label", "Proxy actor address");
      proxyLabel_1.setAttribute("for", "dynamic-argu-key-proxy-actor");
      const proxyActor_1=input("text", "dynamic-argu-actor-address", "dynamic-argu-key-proxy-actor");
      proxyActor_1.setAttribute("id", "dynamic-argu-key-proxy-actor");
      proxyActor_1.setAttribute("placeholder", "akka.tcp://PtcsHost@127.0.0.1:9779/user/actor-argu-proxy");
      proxyActor_1.value=defaultProxyAddress_1;
      const targetLabel=element("label", "dynamic-argu-label", "Target actor address");
      targetLabel.setAttribute("for", "dynamic-argu-key-target-actor");
      const targetActor=input("text", "dynamic-argu-actor-address", "dynamic-argu-key-target-actor");
      ...
      argInput.value=dynamicCanonicalArgFromKeyParts(defaultKeyParts_1);
      ...
      clean_1.addEventListener("click", () => {
        ...
        argInput.value="";
        renderTargetConfig();
        return proxyActor_1.focus();
      });
      cancel_2.addEventListener("click", ctx.cancelKey);
      submit_1.addEventListener("click", () => {
        let keyTail;
        const proxyAddress=Trim(proxyActor_1.value);
        const targetAddress=Trim(targetActor.value);
        ...
        return isBlank(proxyAddress)?proxyActor_1.focus():isBlank(targetAddress)?targetActor.focus():isBlank(selectedTypeName)?typeInput.focus():tryFindDocument(selectedTypeName)!=null||length(keyTail)>0?ctx.submitKey(New_3(ofSeq(delay(() => append_2([proxyAddress], delay(() => append_2([explicitTargetMarker()], delay(() => append_2([targetAddress], delay(() => append_2([selectedTypeName], delay(() => keyTail)))))))))), displayName)):null;
      });
      append(actions_1, [clean_1, cancel_2, submit_1]);
      append(root_1, [proxyLabel_1, proxyActor_1, targetLabel, targetActor, typeLabel, typeInput, aliasLabel_1, aliasInput_1, targetConfig, actions_1]);
      return Some(root_1);
    }
  }
}
...
function renderAppendInput(ctx){
  ...
  const fromKey=dynamicTypeNameFromKeyParts(keyParts);
  const typeName=isBlank(fromKey)?asText(ctx.duTypeName):fromKey;
  const resolveKeyParts=resolveKeyPartsFromSelectedKey(keyParts);
  const isBackendTarget=length(resolveKeyParts)===3&&!isBlank(get(resolveKeyParts, 2));
...
function explicitTargetMarker(){
  return _c_2.explicitTargetMarker;
}
function dynamicCanonicalArgFromKeyParts(keyParts){
  const keyParts_1=normalizeDynamicTargetKeyParts(keyParts);
  return isExplicitTargetKey(keyParts_1)&&length(keyParts_1)>4?asText(get(keyParts_1, 4)):length(keyParts_1)>2?asText(get(keyParts_1, 2)):"";
}
function dynamicTypeNameFromKeyParts(keyParts){
  const keyParts_1=normalizeDynamicTargetKeyParts(keyParts);
  return isExplicitTargetKey(keyParts_1)&&length(keyParts_1)>3?asText(get(keyParts_1, 3)):length(keyParts_1)>1?asText(get(keyParts_1, 1)):"";
}
function isExplicitTargetKey(keyParts){
  const keyParts_1=map(asText, arrayOrEmpty(keyParts));
  return length(keyParts_1)>=4&&isExplicitTargetMarker(get(keyParts_1, 1));
}
function schemaKeys(){
...
function normalizeDynamicTargetKeyParts(keyParts){
  const keyParts_1=map(asText, arrayOrEmpty(keyParts));
  if(isExplicitTargetKey(keyParts_1))return keyParts_1;
  else if(length(keyParts_1)===3){
...
function resolveKeyPartsFromSelectedKey(keyParts){
  const keyParts_1=normalizeDynamicTargetKeyParts(keyParts);
  return isExplicitTargetKey(keyParts_1)?filter((x) =>!isBlank(x), [length(keyParts_1)>2?asText(get(keyParts_1, 2)):"", length(keyParts_1)>3?asText(get(keyParts_1, 3)):"", length(keyParts_1)>4?asText(get(keyParts_1, 4)):""]):keyParts_1;
}
...
function renderSchemaIntoRoot(root, context, typeName, document, schema){
  root.textContent="";
  const readDefaults=defaultValuesReader(defaultsFromDocument(document));
  ...
  if(length(unionCases)===0)root.appendChild(errorNode("Dynamic Argu schema has no requested union cases for DU type: "+typeName));
    ...
    if(!isDocumentBacked)root.appendChild(renderComposerModeControl(context));
    ...
    iter((unionCase) => {
      ...
      fieldGetters=map((field_1) => {
        const p=renderField(refreshPreview, (_1, _2) =>(readDefaults(_1))(_2), caseName, field_1);
    ...
    if(isDocumentBacked){
      if(fullPreviewOpt!=null&&fullPreviewOpt.$==1){
        if(fullSendOpt!=null&&fullSendOpt.$==1){
          ...
          append(root, [fullPreview, actions]);
          refreshFullPreview();
        }
        else void 0;
      }
...
function renderFallbackSchemaWithWarning(root, context, typeName, message){
  let _1;
  const document=tryFindDocument(typeName);
  const fallbackContext=isExplicitTargetKey(context.keyParts)?New_4(context.shape, context.selectedKeyJson, context.selectedKeys, context.keyParts, context.actorAddress, context.duTypeName, [], context.submit, context.composerMode, context.setComposerMode):context;
  const schema=document!=null&&document.$==1&&(!(document.$0.arguFormSchema==null)&&(_1=document.$0,true))?Some(_1.arguFormSchema):tryFindSchema(typeName);
  if(schema==null){
    root.textContent="";
...
function isExplicitTargetMarker(value){
  return Trim(asText(value)).toLowerCase()==explicitTargetMarker();
}
...
function renderComposerModeControl(context){
  const group_1=setTestId("append-composer-mode", element("div", "append-composer-mode", null));
  group_1.setAttribute("data-mode", "form");
  const plain=button("append-composer-mode-button", "append-composer-mode-plain", "Plain");
  const formButton=button("append-composer-mode-button", "append-composer-mode-form", "Form");
  plain.setAttribute("aria-pressed", "false");
  formButton.setAttribute("aria-pressed", "true");
  plain.addEventListener("click", () => context.setComposerMode("plain"));
  formButton.addEventListener("click", () => context.setComposerMode("form"));
  append(group_1, [plain, formButton]);
  return group_1;
}
...
function renderField(refresh, readDefaults, caseName, field_1){
  let getter, _1;
  ...
  const fieldDefaults=readDefaults(caseName, field_1.name);
  getter=() =>[];
  ...
  switch(m){
    ...
    case"tuple":
      ...
      _1=(iteri((_5, _6) => {
        ...
        const testId="";
        const defaults=readDefaults(caseName, _6.name);
...
function defaultValuesReader(defaultMap){
  return reader(defaultMap);
}
function defaultsFromDocument(document){
  return document!=null&&document.$==1?document.$0==null?(document.$0,new FSharpMap("New", [])):group(ofSeq(choose_1((node) => {
...
function mountAppendPage(page, definition){
  ...
  rerenderAddKeyBuilder=() => {
    ...
    const rendererShape=asText_2(addKeyMode).toLowerCase()=="target"?baseRendererShape=="actor-dynamic"?"actor-dynamic-target":baseRendererShape=="actor-argu"?"actor-argu-target":baseRendererShape:baseRendererShape;
  ...
  rerenderAppendForm=() => {
    ...
    const n=setData("selected-key-source", isBlank_2(effectiveKeyJson)?"none":"selected", x);
    setData("composer-mode", composerMode, n);
    const setComposerMode=(nextMode) => {
      const normalized=sameText(asText_2(nextMode), "form")?"form":"plain";
    ...
    const renderPlainComposer=() => {
      ...
      const group_1=setData("mode", composerMode, n_1);
      ...
      let _4=(plain.setAttribute("aria-pressed", sameText(composerMode, "plain")?"true":"false"),formButton.setAttribute("aria-pressed", sameText(composerMode, "form")?"true":"false"),plain.addEventListener("click", () => setComposerMode("plain")),formButton.addEventListener("click", () => setComposerMode("form")),append_1(group_1, [plain, formButton]),group_1);
      let _5=[_4, appendButton];
      append_1(_3, _5);
      append_1(form, [valueInput, actions]);
    };
    const customNode=!sameText(composerMode, "form")||isBlank_2(effectiveKeyJson)?null:tryRenderAppendInputWithRegisteredRenderers(definition.pageId, rendererShape, definition.title, definition.setName, effectiveKeyId, effectiveKeyJson, selectedKeys, valueInput.placeholder, valueInput.value, (payload) => {
      let _3;
    ...
    else isActorArguPage(definition)||isActorDynamicPage(definition)?renderPlainComposer():asText_2(definition.shape).toLowerCase()=="fcell-chat"?(form.className="append-form chat-form",append_1(form, [directionInput, valueInput, appendButton])):(form.className="append-form",append_1(form, [valueInput, appendButton]));
  };
  rerenderAddKeyBuilder();
  rerenderAppendForm();
    ...
    try {
      ...
      switch(responseStatus=="ok"?responseType=="subscribe"?0:responseType=="stream-event"?1:responseType=="read-tail"?2:responseType=="read"?2:responseType=="tail"?2:4:responseStatus=="error"?3:4){
        ...
        case 2:
          ...
          try {
            _1=iter(handleSyncEvent, arrayOrEmpty_1(response.events));
          }
          finally {
            batchingSetSyncEvents=false;
          }
...
function mountManagement(page){
  ...
  const participantHideSelected=setTestId_1("management-participants-hide-selected", button_1("", "Hide selected"));
  const participantDeleteSelected=setTestId_1("management-participants-delete-selected", button_1("management-delete", "Delete selected"));
  ...
  append_1(participantSelectionBar, [participantSelectionCount, participantShowSelected, participantHideSelected, participantDeleteSelected]);
  ...
  const updateParticipantSelectionControl=() => {
    ...
    updateHeaderSelection("management-participants-select-current", map((a) => a.participantId, filter(participantCanSelect, currentParticipantRows())), selectedParticipantKeys);
  };
  ...
  function loadParticipants(){
    ...
    getJson("/management/api/participants", (reply) => {
      ...
      selectedParticipantKeys=filter((selected) => exists((y) => selected==y, availableKeys), selectedParticipantKeys);
  ...
  function deleteSelectedParticipants(){
    ...
    if(length(rows)>0&&globalThis.confirm("Delete "+String(length(rows))+" selected participants, their inbound direct messages, and participant-scoped set projections?")){
      ...
      postJson_2("/management/api/participants/delete-many", New_33(map((row) => New_32(row.participantId), rows)), () => {
        ...
        setStatus(participantCount, "Delete selected failed: "+error);
      });
    }
  }
  ...
  const participantHeaderSelector=Doc.Element("input", [Attr.Create("type", "checkbox"), Attr.Create("id", "management-participants-select-current"), Attr.Create("class", "management-row-selector"), Attr.Create("aria-label", "Select all participants on the current page"), Attr.Create("title", "Select or deselect the current page"), Attr.Create("data-testid", "management-participants-select-current"), Handler("change", (element_2) =>() => {
    selectedParticipantKeys=updateCurrentSelection(element_2.checked, map((a) => a.participantId, filter(participantCanSelect, currentParticipantRows())), selectedParticipantKeys);
    iter((row) => {
...
function RegisterReplyPresentation(rendererId, tryResolve_1){
  ...
  const _1=rendererId_1;
  const _2=tryResolve_1;
  if(globalThis.PulseTradeRegisterReplyPresentation)globalThis.PulseTradeRegisterReplyPresentation(_1, 0, _2);
}
...
function renderAppendValue(definition, value){
  ...
  if(m_1!=null&&m_1.$==1){
    ...
    for(let i=0, _4=presentationActions.length-1;i<=_4;i++)((() => {
      ...
      const actionButton=setData("action-id", asText_2(action.ActionId), setTestId_1("reply-presentation-extension-action", button_1("reply-presentation-extension-action", textOr("Action", action.Label))));
      ...
      actionButton.addEventListener("click", (event) => {
        ...
        try {
          ...
          actionFeedback.className="reply-presentation-action-feedback error";
          actionFeedback.textContent=textOr("Action failed", errorMessage_2(error));
          return;
        }
      });
...
function initializeClientExtensionGlobals(){
  ...
  globalThis.PulseTradeRegisterReplyPresentation=(name, priority, func) => {
    register_1(globalThis.PulseTrade.ReplyPresentationResolvers, name, priority, func);
  };
}
...
function register(){
  return RegisterReplyPresentation("dynamic-static-sdui-v2", tryResolve);
}
...
let _c_2=Lazy((_i) => class $StartupCode_ArguFormRenderer {
  ...
  static {
    ...
    this.schemas=[];
    this.documents=[];
    this.explicitTargetMarker="target-v1";
  }
});
function Get(x){
