      ...
      if(x==null)void 0;
        ...
        if(!isBlank(nodeId)&&!isBlank(actorId)){
          ...
          if(_2!=null&&_2.$==1?incomingGeneration!=null&&incomingGeneration.$==1?!sameText(_2.$0, incomingGeneration.$0)?(_2.$0,incomingGeneration.$0,incomingEventKind==null?false:sameText(incomingEventKind.$0, "Registered")):true:true:true){
            const actor=New_26(actorId, textOr(actorId, _1.displayName), textOr("actor", _1.kind), [nodeId, actorId].concat(tags), textOr("running", _1.status), arrayOrEmpty(_1.routees));
            if(existingNode==null)updatedNode=New_25(nodeId, nodeAddress, actorStatusLooksOffline(actor.status)?"offline":"up", roles, actorStatusLooksOffline(actor.status)?[]:[actor]);
            else {
              ...
              updatedNode=New_25(existing.nodeId, isBlank(nodeAddress)?asText(existing.nodeAddress):nodeAddress, length(actors)===0?"offline":"up", length(roles)===0?arrayOrEmpty(existing.roles):roles, actors);
            }
            ...
            actorSnapshot=New_24(_3, _4, _5, nodes_1);
...
function mountManagement(page){
  ...
  const toggleSelection=(key, isChecked, selected) => isChecked?exists((current) => current==key, selected)?selected:selected.concat([key]):filter_1((current) => current!=key, selected);
  ...
  const updateHeaderSelection=(checkboxId, currentKeys, selected) => {
    ...
    if(!(node==null)){
      const selectedCount=filter_1((key) => exists((y) => key==y, selected), currentKeys).length;
  ...
  const updateCurrentSelection=(isChecked, currentKeys, selected) => isChecked?distinct(selected.concat(currentKeys)):filter_1((key) =>!exists((y) => Equals(key, y), currentKeys), selected);
  ...
  function mutatePage(endpoint){
    return(action) =>(row) => {
      ...
      return!destructive||globalThis.confirm("Delete tab page '"+textOr(row.pageId, row.title)+"'? This cannot be undone for this page lineage.")?(setStatus(pageCount, action+" "+row.pageId+"..."),postJson(endpoint, New_27(row.pageId, row.tabId), () => {
  ...
  function deleteSelectedPages(){
    ...
    if(length(rows)>0&&globalThis.confirm("Delete "+String(length(rows))+" selected tab page lineages? This cannot be undone.")){
      ...
      postJson("/management/api/pages/delete-many", New_28(map((row) => New_27(row.pageId, row.tabId), rows)), () => {
        selectedPageKeys=[];
        updatePageSelectionControl();
        loadPages();
        refreshManagementNav();
  ...
  function deleteSelectedSets(){
    ...
    if(length(rows)>0&&globalThis.confirm("Delete "+String(length(rows))+" selected set buckets from the current projection?")){
      ...
      postJson("/management/api/sets/delete-many", New_29(map((row) => New_30(row.setName, arrayOrEmpty(row.keys)), rows)), () => {
        selectedSetKeys=[];
        updateSetSelectionControl();
        loadSets();
      }, (error_3) => {
  ...
  function mutateParticipant(endpoint){
    return(action) =>(row) => {
      const destructive=EndsWith(endpoint, "/delete");
      return!destructive||globalThis.confirm("Delete participant '"+row.participantId+"' and existing inbound direct messages?")?(setStatus(participantCount, action+" "+row.participantId+"..."),postJson(endpoint, New_32(row.participantId), () => {
        let _1;
        if(destructive){
          const x=row.participantId;
  ...
  function deleteSelectedParticipants(){
    ...
    if(length(rows)>0&&globalThis.confirm("Delete "+String(length(rows))+" selected participants, their inbound direct messages, and participant-scoped set projections?")){
      ...
      postJson("/management/api/participants/delete-many", New_33(map((row) => New_32(row.participantId), rows)), () => {
        selectedParticipantKeys=[];
        updateParticipantSelectionControl();
        loadParticipants();
        loadSets();
...
function mountChat(page){
  ...
  const persistActivity=() => {
    writeJson(activityCacheKey, New_36(activityCursors, unreadMessages));
  };
  ...
  function pollActivity(){
    if(activityReady&&!activityPolling){
      ...
      postJson("/chat/api/activity", New_38(activityCursors), (reply) => {
        ...
        iter((stream) => {
          if(!(stream==null)&&!isBlank(stream.target)){
            ...
            if(!Equals(_2, Some(stream.nextSequence))){
              activityCursors=filter_1((row) =>!sameText(row.target, stream.target), activityCursors).concat([New_39(stream.target, stream.nextSequence)]);
              changed=true;
            }
            iter((event) => {
              if(!(event==null)&&!isBlank(event.target)&&!isBlank(event.messageId)){
                ...
                if(!Equals(_3, Some(event.messageId))){
                  unreadMessages=filter_1((row) =>!sameText(row.target, event.target), unreadMessages).concat([New_35(event.target, event.messageId)]);
                  changed=true;
  ...
  function mutateSelectedGroup(url, participantId_1, displayName, role, historyPolicy, includeHistory, onOk){
    if(selectedGroup!=null&&selectedGroup.$==1){
      ...
      return postJson(url, New_31(newRequestId("group-mutation"), group.groupId, group.revision, asText(participantId_1), asText(displayName), asText(role), asText(historyPolicy), includeHistory), (reply) => {
    ...
    if(selectedGroup!=null&&selectedGroup.$==1){
      ...
      if(isAnnouncement&&currentMember!=null&&!readOnlyView){
        ...
        _1=(markRead.addEventListener("click", () => {
          ...
          return latest!=null&&latest.$==1?postJson("/chat/api/groups/read/ack", New_40(group.groupId, latest.$0.channelMessageId), (reply) => {
        ...
        getJson(url, (data) => {
          ...
          readJson(cacheKey_1, (cached) => {
            ...
            readWatermark(cacheKey_1, (watermark) => {
              ...
              let _3=Compare(a, b)===1?a:b;
              writeSnapshotWithWatermark(cacheKey_1, New_41(merged, nextAfterMessageId, storedOldestSequence, storedHasOlderMessages), _3, length(merged), "chat-thread");
            });
          });
  ...
  function cacheAcceptedChatMessage(sequence_1, message){
    if(!(message==null)&&!isBlank(message.messageId)&&!isBlank(selected)){
      ...
      return readJson(cacheKey_1, (cached) => {
        ...
        let _3=New_41(merged, message.messageId, _1, _2);
        ...
        if(responseType=="subscribe")setChatWsState("subscribed");
          ...
          if(!isBlank(selected)&&!(event==null)&&!(event.streamKey==null)&&streamIdentity(event.streamKey)==streamIdentity(chatStreamKey(selected))){
            ...
            if(event_1==null||isBlank(event_1.payload))o=null;
              ...
              o=Some(New_37(textOr(event_1.eventId, event_1.sourceId), "", 0n, "", participantId, "direct", asText(event_1.payload), asText(event_1.createdAtUtc)));
            }
            if(o==null)null;
            else {
              const message_1=o.$0;
  ...
  function sendMessage(){
    ...
    if(isBlank(selected))setStatus(state, "Select a participant first");
      ...
      const request=New_43(newRequestId("group-send"), selectedGroupId(), body, ["web-chat"]);
      ...
      const request_1=New_45(participantId, selected, body, ["web-chat"]);
      const pendingId_1=rememberPending("chat-send", participantId+"->"+selected, "/chat/api/send", request_1);
      const wsRequest=New_44("chat-send", pendingId_1, participantId, selected, body, ["web-chat"], participantId, "chat");
  ...
  function exportSelectedThread(){
    if(isBlank(selected))setStatus(state, "Select a participant first");
      ...
      try {
        const rows=map((message) => New_46(asText(message.messageId), asText(message.fromId), asText(message.createdAtUtc), asText(message.body)), selectedThreadMessages);
  ...
  groupCreateConfirm.addEventListener("click", () => {
    ...
    return isBlank(groupId)?setStatus(state, "Group id is required"):postJson("/chat/api/groups/create", New_47(newRequestId("group-create"), groupId, displayName, [], groupHistoryInput.value, ["web-chat"]), (reply) => {
      const createdTarget=groupTarget(reply.group.groupId);
      if(!exists((row) => sameText(row.target, createdTarget), activityCursors)){
        activityCursors=activityCursors.concat([New_39(createdTarget, "0")]);
        persistActivity();
      }
...
function loginConfig(){
  ...
  return node==null||isBlank(node.textContent)?New_49("/login/api/submit", "/login/api/session", "/login/logout", "/actors", "/actors", "ptc_login_session", "\u767b\u5165 PTCS", "\u4f7f\u7528 host \u63d0\u4f9b\u7684\u5e33\u865f\u767b\u5165\u3002\u6b0a\u9650\u7531\u767b\u5165\u5f8c\u53d6\u5f97\u7684 principal \u8207 ACL policy \u6c7a\u5b9a\u3002", "PTCS.Login", "ACL mode"):json(node.textContent);
}
...
function renderAppendValue(definition, value){
  ...
  const presentationContext=New_51(asText(definition.pageId), asText(definition.tabId), asText(value.valueId), asText(value.createdAtUtc), mode, arrayOrEmpty(value.tags), asText(value.rawValue));
...
function renderViewAsControl(){
  ...
  const apply=setTestId("view-as-apply", button("primary", "Apply"));
...
function renderPageCreator(nav, activePath, pages){
  ...
  const addPageAfterCandidates=() => {
    ...
    if(isBlank(pageIdText)&&isBlank(titleText))setStatus(status, "Page id or title is required");
      ...
      const request=New_53(pageIdText, titleText, "", shape.value, p[0], p[1], "", "");
...
function currentBrowserUser(){
  ...
  if(userNode==null||isBlank(userNode.textContent))return New_34("user.web", "Web User", "", false, "anonymous", "/chat/logout", "user.web", "", false);
  else {
    const user=json(userNode.textContent);
    return user==null||isBlank(user.participantId)?New_34("user.web", "Web User", "", false, "anonymous", "/chat/logout", "user.web", "", false):user;
  }
}
...
function shapeRegistration(shape, label, badge, className){
  return New_48(normalizeShapeText(shape), textOr(normalizeShapeText(shape), label), textOr("?", badge), textOr(normalizeShapeText(shape), className));
}
...
function mountCore(mountDocument, extensionId, channelId, canvasId, lifecycleOptions, disposeAfterJsonExport){
  ...
  const sendPayloadWithRequestId=(requestId, operation, payload) => {
    const text_1=JSON.stringify(New_54("extension-transient", requestId, extensionId, channelId, operation, JSON.stringify(payload)));
...
function downloadJsonExport(wire){
  if(globalThis.document.body==null)return Error_1("Document body is unavailable.");
    ...
    const url=URL.createObjectURL(new Blob([JSON.stringify(New_58("ptcs-ta-research-export.v1", (new Date()).toISOString(), wire.documentRevision, wire.dataRevision, wire))], {type:"application/json;charset=utf-8"}));
    const anchor=globalThis.document.createElement("a");
    anchor.setAttribute("href", url);
...
function initial(canvasInstanceId){
  return New_55(canvasInstanceId, {$:0}, false, false, true, false, 0n, 0, false, false);
}
function transition(options, event, state){
  ...
  if(state.Disposed&&event.$!==9)return[state, []];
    ...
    case 0:
      return[New_55(state.CanvasInstanceId, {$:7}, event.$1, state.Connected, state.Active, true, event.$0, state.ReconnectAttempt, state.DisposePending, state.Disposed), [CancelPoll, CancelTimeout, CancelReconnect, SendUnmounted, ScheduleTimeout(options.RequestTimeoutMs)]];
    case 1:
      return[New_55(state.CanvasInstanceId, {$:7}, state.PollEnabled, false, state.Active, false, state.DataRevision, state.ReconnectAttempt, false, true), [CancelPoll, CancelTimeout, CancelReconnect, CloseTransport]];
    case 2:
      return[state, []];
    case 3:
      return[state, []];
    ...
    case 0:
      return[New_55(state.CanvasInstanceId, {$:1}, state.PollEnabled, true, state.Active, true, state.DataRevision, 0, state.DisposePending, state.Disposed), [CancelReconnect, SendMounted, ScheduleTimeout(options.RequestTimeoutMs)]];
    case 1:
      const pollEnabled=_1[0];
      return[New_55(state.CanvasInstanceId, state.Active&&pollEnabled?{$:2}:{$:5}, pollEnabled, state.Connected, state.Active, false, _1[1], state.ReconnectAttempt, state.DisposePending, state.Disposed), [CancelTimeout].concat(state.Active&&pollEnabled?[SchedulePoll(options.PollIntervalMs)]:[])];
    case 2:
      return[New_55(state.CanvasInstanceId, state.Active&&state.PollEnabled?{$:2}:{$:5}, state.PollEnabled, state.Connected, state.Active, false, state.DataRevision, state.ReconnectAttempt, state.DisposePending, state.Disposed), [CancelTimeout].concat(state.Active&&state.PollEnabled?[SchedulePoll(options.PollIntervalMs)]:[])];
    case 3:
    ...
    case 5:
      ...
      return[New_55(state.CanvasInstanceId, {$:5}, state.PollEnabled, false, state.Active, false, state.DataRevision, attempt, state.DisposePending, state.Disposed), [CancelPoll, CancelTimeout, CancelReconnect, CloseTransport, ScheduleReconnect(reconnectDelay(options, attempt))]];
    case 6:
      return[state, []];
    case 7:
      const attempt_1=state.ReconnectAttempt+1;
      return[New_55(state.CanvasInstanceId, {$:5}, state.PollEnabled, false, state.Active, false, state.DataRevision, attempt_1, state.DisposePending, state.Disposed), [CancelPoll, CancelTimeout, ScheduleReconnect(reconnectDelay(options, attempt_1))]];
    case 8:
...
function writeWatermark(streamId, newestSequence, cachedCount, source){
  if(!isBlank(streamId)){
    ...
    let _5=New_42(streamId, _3, _4, asText(source), nowTicks());
...
class Doc extends Object_1 {
  ...
  static Convert(render_1, view){
    return Doc.Flatten(MapSeqCached(render_1, view));
  }
  static Append(a, b){
...
class ListModel extends Object_1 {
  ...
  constructor(i, _1, _2, _3){
    ...
    if(i=="New"){
      ...
      i="New_3";
      _1=key;
      _2=_c_3.Create_1(ofSeq(distinctBy_1(key, storage.SInit())));
      _3=storage;
    }
    if(i=="New_3"){
      const key_1=_1;
...
function New_27(pageId, tabId){
  return{pageId:pageId, tabId:tabId};
}
function New_28(pages){
  return{pages:pages};
}
function New_29(sets){
  return{sets:sets};
}
function New_30(setName, keys){
...
function New_35(target, messageId){
  return{target:target, messageId:messageId};
}
function New_36(cursors, unread){
...
function New_38(cursors){
  return{cursors:cursors};
}
function New_39(target, sequence_1){
  return{target:target, sequence:sequence_1};
}
function New_40(groupId, channelMessageId){
...
function CreateRunState(parent, doc_1){
  return New_56(get_Empty_1(), CreateElemNode(parent, EmptyAttr(), doc_1));
}
...
function emptyFrame(kind, actionKind, canvasId){
  return New_57("ta-browser.v1", kind, actionKind, canvasId, "", "", "", 0, false, "", "", 0, "", "", false, 0, 0, "", "", false, [], 0, false, "", "", "", "", 0);
}
function actionToWire(action){
  if(action.$==1)return emptyFrame("action", "reset-canvas", canvasText(action.$0));
    ...
    return New_57(_1.wireVersion, _1.kind, _1.actionKind, _1.canvasInstanceId, row.RowId, rowKindText(row.Kind), row.DataRef, row.HeightWeight, row.Visible, _1.sourceId, _1.instrument, _1.intervalMinutes, _1.fromUtc, _1.toUtcExclusive, _1.includePartial, _1.afterDataRevision, _1.dataRevision, _1.reasonCode, _1.templateKey, _1.hasTemplateRowId, _1.editorValues, _1.expectedDocumentRevision, _1.hasExpectedDocumentRevision, _1.baseRowId, _1.eventTimeUtc, _1.startEventTimeUtc, _1.endEventTimeExclusiveUtc, _1.maximumBasePoints);
  }
  else if(action.$==3){
    ...
    return New_57(_2.wireVersion, _2.kind, _2.actionKind, _2.canvasInstanceId, rowId==null?"":rowId.$0, _2.rowKind, _2.dataRef, _2.heightWeight, _2.visible, _2.sourceId, _2.instrument, _2.intervalMinutes, _2.fromUtc, _2.toUtcExclusive, _2.includePartial, _2.afterDataRevision, _2.dataRevision, _2.reasonCode, templateKey, rowId!=null, map(editorInputToWire, values==null?[]:values), _2.expectedDocumentRevision, _2.hasExpectedDocumentRevision, _2.baseRowId, _2.eventTimeUtc, _2.startEventTimeUtc, _2.endEventTimeExclusiveUtc, _2.maximumBasePoints);
  }
  else if(action.$==4){
    const rowId_1=action.$1;
    const _3=emptyFrame("action", "remove-row", canvasText(action.$0));
    return New_57(_3.wireVersion, _3.kind, _3.actionKind, _3.canvasInstanceId, rowId_1, _3.rowKind, _3.dataRef, _3.heightWeight, _3.visible, _3.sourceId, _3.instrument, _3.intervalMinutes, _3.fromUtc, _3.toUtcExclusive, _3.includePartial, _3.afterDataRevision, _3.dataRevision, _3.reasonCode, _3.templateKey, _3.hasTemplateRowId, _3.editorValues, _3.expectedDocumentRevision, _3.hasExpectedDocumentRevision, _3.baseRowId, _3.eventTimeUtc, _3.startEventTimeUtc, _3.endEventTimeExclusiveUtc, _3.maximumBasePoints);
  }
  else if(action.$==5){
    const query=action.$1;
    const _4=emptyFrame("action", "change-query", canvasText(action.$0));
    return New_57(_4.wireVersion, _4.kind, _4.actionKind, _4.canvasInstanceId, _4.rowId, _4.rowKind, _4.dataRef, _4.heightWeight, _4.visible, optionText(query.SourceId), optionText(query.Instrument), optionInt(query.IntervalMinutes), optionText(query.FromUtc), optionText(query.ToUtcExclusive), optionBool(query.IncludePartial), _4.afterDataRevision, _4.dataRevision, _4.reasonCode, _4.templateKey, _4.hasTemplateRowId, _4.editorValues, _4.expectedDocumentRevision, _4.hasExpectedDocumentRevision, _4.baseRowId, _4.eventTimeUtc, _4.startEventTimeUtc, _4.endEventTimeExclusiveUtc, _4.maximumBasePoints);
  }
  else if(action.$==6){
    const change=action.$1;
    const _5=emptyFrame("action", "shared-cursor-changed", canvasText(action.$0));
    return New_57(_5.wireVersion, _5.kind, _5.actionKind, _5.canvasInstanceId, _5.rowId, _5.rowKind, _5.dataRef, _5.heightWeight, _5.visible, _5.sourceId, _5.instrument, _5.intervalMinutes, _5.fromUtc, _5.toUtcExclusive, _5.includePartial, _5.afterDataRevision, _5.dataRevision, _5.reasonCode, _5.templateKey, _5.hasTemplateRowId, _5.editorValues, _5.expectedDocumentRevision, _5.hasExpectedDocumentRevision, change.BaseRowId, change.EventTimeUtc, _5.startEventTimeUtc, _5.endEventTimeExclusiveUtc, _5.maximumBasePoints);
  }
  else if(action.$==7){
    const change_1=action.$1;
    const _6=emptyFrame("action", "visible-range-changed", canvasText(action.$0));
    return New_57(_6.wireVersion, _6.kind, _6.actionKind, _6.canvasInstanceId, _6.rowId, _6.rowKind, _6.dataRef, _6.heightWeight, _6.visible, _6.sourceId, _6.instrument, _6.intervalMinutes, _6.fromUtc, _6.toUtcExclusive, _6.includePartial, _6.afterDataRevision, _6.dataRevision, _6.reasonCode, _6.templateKey, _6.hasTemplateRowId, _6.editorValues, _6.expectedDocumentRevision, _6.hasExpectedDocumentRevision, change_1.BaseRowId, _6.eventTimeUtc, change_1.StartEventTimeUtc, change_1.EndEventTimeExclusiveUtc, change_1.MaximumBasePoints);
  }
  else if(action.$==8){
    const revision=action.$1;
    const _7=emptyFrame("action", "poll-delta", canvasText(action.$0));
    return New_57(_7.wireVersion, _7.kind, _7.actionKind, _7.canvasInstanceId, _7.rowId, _7.rowKind, _7.dataRef, _7.heightWeight, _7.visible, _7.sourceId, _7.instrument, _7.intervalMinutes, _7.fromUtc, _7.toUtcExclusive, _7.includePartial, Number(revision), _7.dataRevision, _7.reasonCode, _7.templateKey, _7.hasTemplateRowId, _7.editorValues, _7.expectedDocumentRevision, _7.hasExpectedDocumentRevision, _7.baseRowId, _7.eventTimeUtc, _7.startEventTimeUtc, _7.endEventTimeExclusiveUtc, _7.maximumBasePoints);
  }
  else if(action.$==9){
    const reason=action.$1;
    const _8=emptyFrame("action", "full-snapshot", canvasText(action.$0));
    return New_57(_8.wireVersion, _8.kind, _8.actionKind, _8.canvasInstanceId, _8.rowId, _8.rowKind, _8.dataRef, _8.heightWeight, _8.visible, _8.sourceId, _8.instrument, _8.intervalMinutes, _8.fromUtc, _8.toUtcExclusive, _8.includePartial, _8.afterDataRevision, _8.dataRevision, reason, _8.templateKey, _8.hasTemplateRowId, _8.editorValues, _8.expectedDocumentRevision, _8.hasExpectedDocumentRevision, _8.baseRowId, _8.eventTimeUtc, _8.startEventTimeUtc, _8.endEventTimeExclusiveUtc, _8.maximumBasePoints);
  }
...
function actionRequestToWire(request){
  ...
  return New_57(_1.wireVersion, _1.kind, _1.actionKind, _1.canvasInstanceId, _1.rowId, _1.rowKind, _1.dataRef, _1.heightWeight, _1.visible, _1.sourceId, _1.instrument, _1.intervalMinutes, _1.fromUtc, _1.toUtcExclusive, _1.includePartial, _1.afterDataRevision, _1.dataRevision, _1.reasonCode, _1.templateKey, _1.hasTemplateRowId, _1.editorValues, _2, request.ExpectedDocumentRevision!=null, _1.baseRowId, _1.eventTimeUtc, _1.startEventTimeUtc, _1.endEventTimeExclusiveUtc, _1.maximumBasePoints);
}
...
function editorInputToWire(input_1){
  ...
  return m.$==1?New_61(input_1.Path, "number", "", m.$0, false):m.$==2?New_61(input_1.Path, "bool", "", 0, m.$0):New_61(input_1.Path, "text", m.$0, 0, false);
}
...
function render(options, callbacks, runtimeState){
  ...
  const cursorIndex=_c_3.Create_1(null);
  ...
  const setUiState=(next) => {
    ...
    uiState.Set(next);
    if(!(Equals(previousChartState.Window, next.Window)&&previousChartState.FollowLatest==next.FollowLatest&&Equals(previousChartState.HiddenRows, next.HiddenRows)))chartUiState.Set(next);
  };
  actionSequence=0;
  querySelectionGeneration=0;
  queryInFlight=false;
  queuedQuery=null;
  boundaryPanGeneration=0;
  ...
  const visibleRangeActionAllowed=(state) => {
    const o_2=state.Document;
    const o_3=o_2==null?null:Some(o_2.$0.AllowedActions);
    let _3=o_3==null?[]:o_3.$0;
    return arrContains("visible-range-changed", _3);
  };
  const viewportCommandsDisabledView=Map2((_3, _4) => visibleRangeActionAllowed(_3)&&(localViewportDisabled(_3.Poll)||_4.PendingActionId!=null), runtimeState.View, uiState.View);
  const viewportCommandsDisabledNow=() => visibleRangeActionAllowed(runtimeState.Get())&&(localViewportDisabled(runtimeState.Get().Poll)||uiState.Get().PendingActionId!=null);
  const startActionWithFeedback=(action, successText, onAccepted, onRejected, afterSettled) => {
    ...
    return submit(callbacks, uiState, runtimeState.Get().DocumentRevision, request, successText, onAccepted, onRejected, afterSettled);
  };
  ...
  const chartRuntimeState=_c_3.Create_1(runtimeState.Get());
  ...
  const scheduleFullPreparation=() => {
    ...
    preparedDataReady=false;
    chartRuntimeState.Set(runtimeState.Get());
    prepareDataScheduled(scheduleNextFrame, runtimeState.Get().Data, (prepared) => {
  ...
  Sink((next) => {
    ...
    if(!Equals(next.Identity, observedDataState.Identity)){
    ...
    else if(!preparedDataReady){
      if(dataChanged){
        observedDataState=next;
        scheduleFullPreparation();
      }
      else observedDataState=next;
    }
    else {
      const nextPreparedData=dataChanged?prepareDataIncremental(latestPreparedData, next.Data):latestPreparedData;
      ...
      if(topologyChanged){
        ...
        if(m==null)_3=pendingBoundaryPan=null;
          ...
          let _6;
          const document=m.$0;
          const nextTimeline=referenceTimelineForDocumentPrepared(document, nextPreparedData);
          const currentUi=uiState.Get();
          const generalOldTimeline=referenceTimelineForDocumentPrepared(document, latestPreparedData);
          const generalOldWindow=resolveWindow(options.MinimumVisibleBars, options.MaximumVisibleBars, length(generalOldTimeline), currentUi.FollowLatest, currentUi.Window);
          if(pendingBoundaryPan!=null&&pendingBoundaryPan.$==1){
            pendingBoundaryPan.$0[4];
            const intentTimeline=pendingBoundaryPan.$0[3];
            const direction=pendingBoundaryPan.$0[1];
            _6=(pendingBoundaryPan.$0,coverageExtended(direction, intentTimeline, nextTimeline)?(_4=[pendingBoundaryPan.$0[2], pendingBoundaryPan.$0[1], pendingBoundaryPan.$0[3], pendingBoundaryPan.$0[4]],0):!currentUi.FollowLatest&&length(generalOldTimeline)>0&&length(nextTimeline)>length(generalOldTimeline)?1:2);
          }
          else _6=!currentUi.FollowLatest&&length(generalOldTimeline)>0&&length(nextTimeline)>length(generalOldTimeline)?1:2;
          switch(_6){
            case 0:
              reanchored=(pendingBoundaryPan=null,tryReanchorWindow(options.MinimumVisibleBars, options.MaximumVisibleBars, _4[0], _4[2], nextTimeline, _4[3]));
              break;
            case 1:
              reanchored=tryReanchorWindow(options.MinimumVisibleBars, options.MaximumVisibleBars, 0, generalOldTimeline, nextTimeline, generalOldWindow);
              break;
            case 2:
              reanchored=null;
              break;
          }
          if(reanchored==null)_3=void 0;
          else {
        ...
        function update(index){
          if(generation===dataWorkGeneration&&index<length(targets))scheduleNextFrame(() => {
  ...
  const setWindow=(followLatest, window_1) => {
    if(!viewportCommandsDisabledNow()){
      const p=commitLocalWindow(followLatest, window_1);
      if(p[0]&&actionAllowed("visible-range-changed")&&!commandsDisabledNow()){
        const m=runtimeState.Get().Document;
        if(m==null)return null;
        else {
          const m_1=visibleEventRangePrepared(m.$0, latestPreparedData, p[1]);
          if(m_1==null)return null;
            ...
            const a=4000;
            const a_1=1;
            const b=options.MaximumVisibleBars;
  ...
  const requestAdjacentCoverage=(direction, delta) => {
    if(actionAllowed("visible-range-changed")&&!commandsDisabledNow()){
      const m=runtimeState.Get().Document;
      if(m==null)return null;
      else {
        const document=m.$0;
        ...
        const m_1=tryAdjacentCoverageRange(direction, _3, document, latestPreparedData);
        if(m_1==null){
          ...
          let _5={
            ...
            Feedback:Equals(direction, {$:0})?"Earlier coverage is outside the configured query boundary.":"Later coverage is outside the configured query boundary."
          };
          return setUiState(_5);
        }
        else {
          const change=m_1.$0;
          const timeline=referenceTimelineForDocumentPrepared(document, latestPreparedData);
          const window_1=resolveWindow(options.MinimumVisibleBars, options.MaximumVisibleBars, length(timeline), uiState.Get().FollowLatest, uiState.Get().Window);
          boundaryPanGeneration=boundaryPanGeneration+1;
          pendingBoundaryPan=Some([boundaryPanGeneration, direction, delta, timeline, window_1]);
          return startActionWith({
            $:7, 
            $0:currentCanvasId(), 
            $1:change
          }, Equals(direction, {$:0})?"Earlier coverage requested.":"Later coverage requested.", () => { }, () => {
            pendingBoundaryPan=null;
          });
        }
      }
  ...
  const panWindow=(delta) => {
    ...
    if(requestedStart<0)requestAdjacentCoverage({$:0}, delta);
    else if(requestedStart>maximumStart)requestAdjacentCoverage({$:1}, delta);
    else {
      const candidate=clampWindow(options.MinimumVisibleBars, options.MaximumVisibleBars, total, {StartIndex:requestedStart, Count:visible.Count});
  ...
  const setElementHidden=(hidden, element_2) => hidden?element_2.setAttribute("hidden", "hidden"):element_2.removeAttribute("hidden");
  const applyVisibleCursorValues=(bounded) => {
    if(!(chartStackElement==null)){
      let _3;
      const hint=tryHead(cursorElements("[data-ta-cursor-hint]"));
      const time=tryHead(cursorElements("[data-ta-cursor-time]"));
      const valueNodes=cursorElements("[data-ta-cursor-value-index]");
      const legendValueNodes=cursorElements("[data-ta-row-value-index]");
      const rowTimeNodes=cursorElements("[data-ta-row-data-time='true']");
      const legendIndex=bounded==null?length(latestCursorTimestamps)>0?Some(length(latestCursorTimestamps)-1):null:Some(bounded.$0);
      for(let valueIndex=0, _4=length(legendValueNodes)-1;valueIndex<=_4;valueIndex++)((() => {
        let o_2, o_3;
        const node_1=get(legendValueNodes, valueIndex);
        const m=(o_2=0,[TryParse(node_1.getAttribute("data-ta-row-value-index"), {get:() => o_2, set:(v) => {
          o_2=v;
        }}), o_2]);
        ...
        if(legendIndex==null)o_3=null;
          ...
          o_3=tryLegendValue(latestLegendReaders, rowId, traceIndex, c);
        }
        const o_4=o_3==null?null:Some(o_3.$0.Value);
        const nextValue=o_4==null?"Unavailable":o_4.$0;
        node_1.textContent=nextValue;
        return node_1.setAttribute("data-value-state", nextValue=="Unavailable"?"undefined":"defined");
      })());
      for(let i=0, _5=rowTimeNodes.length-1;i<=_5;i++)((() => {
        let o_2;
        const node_1=get(rowTimeNodes, i);
        const rowId=node_1.getAttribute("data-ta-row-data-time-row-id");
        if(legendIndex==null)o_2=null;
        else {
          const c=legendIndex.$0;
          o_2=tryRowPresentation(latestLegendReaders, rowId, c);
        }
        const o_3=o_2==null?null:fullTimestamp(o_2.$0.Timestamp);
        let _6=o_3==null?"Unavailable":o_3.$0;
        node_1.textContent=_6;
      })());
      if(bounded!=null&&bounded.$==1){
        const index=bounded.$0;
        if(hint==null){ }
        else setElementHidden(true, hint.$0);
        if(time==null)_3=void 0;
        else {
          const node=time.$0;
          _3=(node.textContent=compactTimestamp(get(latestCursorTimestamps, index)),setElementHidden(false, node));
        }
        for(let valueIndex_1=0, _6=length(valueNodes)-1;valueIndex_1<=_6;valueIndex_1++)(((valueIndex_2) => {
          const node_1=get(valueNodes, valueIndex_1);
          const o_2=tryItem(valueIndex_2, latestCursorReaders);
          const m=o_2==null?null:o_2.$0(index);
          if(m==null){
            node_1.textContent="";
            node_1.removeAttribute("data-cursor-row");
            return setElementHidden(true, node_1);
          }
          else {
            const current=m.$0;
            node_1.textContent=current.Label+" "+current.Value;
            node_1.setAttribute("data-cursor-row", current.Label);
            return setElementHidden(false, node_1);
          }
        })(valueIndex_1));
      }
      else {
        if(hint==null){ }
        else setElementHidden(false, hint.$0);
        if(time==null){ }
        else setElementHidden(true, time.$0);
        for(let i_1=0, _7=valueNodes.length-1;i_1<=_7;i_1++)setElementHidden(true, get(valueNodes, i_1));
      }
    }
  };
  refreshVisibleValues=() => {
  ...
  const applyCursorIndex=(value) => {
    if(!(chartStackElement==null)){
      ...
      if(m==null){
        for(let i=0, _8=crosshairs.length-1;i<=_8;i++)get(crosshairs, i).setAttribute("visibility", "hidden");
        for(let i_1=0, _9=rowCursorLabels.length-1;i_1<=_9;i_1++)get(rowCursorLabels, i_1).setAttribute("visibility", "hidden");
        _3=void 0;
      }
      else {
        const x=m.$0;
        const xText=fixedText(x);
        for(let i_2=0, _10=crosshairs.length-1;i_2<=_10;i_2++){
        ...
        const labelX=fixedText(_7);
        for(let i_3=0, _11=rowCursorLabels.length-1;i_3<=_11;i_3++)((() => {
          let o_3;
          const group=get(rowCursorLabels, i_3);
          const rowId=group.getAttribute("data-ta-row-cursor-row-id");
          if(bounded==null)o_3=null;
          else {
            const c=bounded.$0;
            o_3=tryRowPresentation(latestLegendReaders, rowId, c);
          }
          const o_4=o_3==null?null:timestampParts(o_3.$0.Timestamp);
          const p=o_4==null?["Unavailable", "Unavailable"]:o_4.$0;
  ...
  const commitCursorIndex=(index) => {
    ...
    if(actionAllowed("shared-cursor-changed")&&!commandsDisabledNow()){
      ...
      if(m==null){ }
        ...
        const document=m.$0;
        const timeline=referenceTimelineForDocument(document, runtimeState.Get().Data);
  ...
  return Doc.Element("div", [Attr.Create("class", "ptcs-ta-workspace"), Attr.Create("data-testid", "ta-workspace"), Attr.Create("style", "display:flex; flex-direction:column; min-width:0; width:100%; min-height:640px; color:#142033; background:#f4f7fb; font-family:Segoe UI, Arial, sans-serif; letter-spacing:0;")], [Doc.EmbedView(MapCachedBy(sameDocumentShell, (state) => {
    ...
    if(m!=null&&m.$==1){
      ...
      return Doc.Element("div", [Attr.Create("style", "display:flex; flex-direction:column; min-width:0;")], [Doc.Element("header", [Attr.Create("style", "display:flex; flex-direction:column; gap:7px; padding:10px 12px 8px; background:#fff; border-bottom:1px solid #dbe3ee;")], [Doc.Element("div", [Attr.Create("style", "display:flex; align-items:center; justify-content:space-between; gap:10px; flex-wrap:wrap;")], [Doc.Element("div", [Attr.Create("style", "min-width:0;")], [Doc.Element("h2", [Attr.Create("data-testid", "ta-workspace-title"), Attr.Create("style", "margin:0; font-size:17px; line-height:22px; font-weight:700; color:#152944;")], [Doc.TextNode(document.Title)]), Doc.Element("div", [Attr.Create("data-testid", "ta-canvas-identity"), Attr.Create("style", "font-size:11px; color:#667891; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;")], [Doc.TextView(Map((current) =>"canvas "+canvasIdText(current.Identity.CanvasInstanceId)+" / revision "+String(current.DataRevision), runtimeState.View))])]), Doc.EmbedView(Map((current) => {
        ...
        toDateDraft=value;
      })]), primaryButtonView("ta-apply-query", "Load / Apply", Map((state_1) => remoteDisabled(state_1.Poll), runtimeState.View), () => remoteDisabled(runtimeState.Get().Poll), applyQuery)]), Doc.Element("div", [Attr.Create("data-testid", "ta-local-toolbar"), Attr.Create("style", "display:flex; align-items:center; gap:5px; flex-wrap:wrap;")], append_1(ofArray([compactRemoteButton("ta-pan-left", "\u2190", "Pan earlier", viewportCommandsDisabledView, viewportCommandsDisabledNow, () => {
        const a=1;
      ...
      })]):FSharpList.Empty, ofArray([Doc.Element("span", [Attr.Create("style", "margin-left:auto; color:#60738b; font-size:11px;")], [Doc.TextNode("viewport changes request the selected event-time range when enabled")])])))), Doc.EmbedView(Map((ui) => Doc.Element("div", [Attr.Create("data-testid", "ta-row-toggles"), Attr.Create("style", "display:flex; align-items:center; gap:5px; flex-wrap:wrap;")], ofSeq_1(delay(() => collect_1((row) => {
        let editable;
        const hidden=ui.HiddenRows.Contains(row.RowId);
        const displayLabel=rowDisplayLabel(row);
        ...
        })], [Doc.TextNode(displayLabel)])], delay(() => append_2(editable?[Doc.Element("button", [Attr.Create("type", "button"), Attr.Create("data-testid", "ta-edit-row-"+row.RowId), Attr.Create("title", "Edit "+displayLabel+" parameters"), DynamicBool("disabled", commandsDisabledView), Dynamic_1("style", Map((disabled) => disabled?"height:26px; border:1px solid #c8d2df; border-right:0; background:#edf1f5; color:#8b98a8; padding:2px 7px; font-size:11px; cursor:not-allowed;":"height:26px; border:1px solid #9cb3cc; border-right:0; background:#fff; color:#315d88; padding:2px 7px; font-size:11px; cursor:pointer;", commandsDisabledView)), Handler("click", () =>() => {
        ...
        function mountRow(index){
          if(workGeneration===chartWorkGeneration&&index<length(visibleRows))scheduleNextFrame(() => {
            if(workGeneration===chartWorkGeneration){
              ...
              latestLegendReaders=OfArray(choose((x) => x, mapi((_15, _16) => _16==null?null:Some([get(visibleRows, _15).RowId, _16.$0]), stagedLegendReaders)));
...
function renderRowReactivePreparedLiveWithValueRefresh(_1, _2, preparedData, dataView, visibleTimestamps, cursorIndex, setCursorIndex, commitCursorIndex, showSharedTimeAxis, isBaseRow, scheduleValueRefresh, row){
  ...
  return[chartFrame(title, [Doc.EmbedView(Map((currentData) => Doc.Element("span", [Attr.Create("style", "display:inline-flex; align-items:center; gap:6px 10px; flex:0 0 auto; flex-wrap:nowrap; white-space:nowrap;")], ofSeq_1(delay(() => collect_1((value) => {
    ...
    return append_2([Doc.Element("span", [Attr.Create("data-testid", "ta-row-data-time-"+row.RowId), Attr.Create("data-ta-row-data-time", "true"), Attr.Create("data-ta-row-data-time-row-id", row.RowId), Attr.Create("style", "display:inline-block; width:19ch; min-width:19ch; max-width:19ch; overflow:hidden; white-space:nowrap; font-variant-numeric:tabular-nums; font-weight:650;")], [Doc.TextNode(initialTimestamp)])], delay(() => collect_1((index) => {
      let initialValue;
      ...
      if(!Equals(trace.Kind, {$:4})){
        ...
        if(length(timestamps)===0)initialValue="Unavailable";
          ...
          const o_2=(get(legendReaders, index))(length(timestamps)-1);
...
function sameDocumentShell(left, right){
  ...
  const samePresence=_1!=null&&_1.$==1?_2!=null&&_2.$==1&&_1.$0.WorkspaceId==_2.$0.WorkspaceId:_2==null;
  return Equals(left.Identity, right.Identity)&&samePresence&&left.DocumentRevision===right.DocumentRevision;
}
function element_1(name, attrs, children){
  return Doc.Element(name, attrs, children);
}
function axisResizeBound(){
  return _c_5.axisResizeBound;
}
function set_axisResizeBound(_1){
  _c_5.axisResizeBound=_1;
}
function axisViewportWidth(){
  return _c_5.axisViewportWidth;
}
...
function compositeSvgReactivePreparedLiveWithValueRefresh(rowId, isBaseRow, traces, preparedData, dataView, referenceTimestamps, _1, setCursorIndex, commitCursorIndex, scheduleValueRefresh){
  ...
  const prepareGeometry=(currentData) => {
    ...
    for(let i=0, _2=preparedTraces.length-1;i<=_2;i++){
      const f=get(preparedTraces, i);
      const trace=f[1];
      const candles=f[2];
      if(Equals(trace.Kind, {$:0})){
        for(let i_1=0, _3=candles.length-1;i_1<=_3;i_1++){
          const point=get(candles, i_1);
          const m=candleSlotRange(referenceTimestamps, point);
          if(m==null)void 0;
          else {
            const lastExclusive=m.$0[1];
            const first=m.$0[0];
            for(let slotIndex=first, _4=lastExclusive-1;slotIndex<=_4;slotIndex++)projected.push([f[0], trace, slotIndex, lastExclusive-first, point]);
          }
        }
      }
      else void 0;
    }
    const projectedCandleSeries=projected.slice();
    const candleSeries=length(referenceTimestamps)<=maximumVisualPoints?projectedCandleSeries:sortBy((_5) =>[_5[0], _5[2]], map((_5) => {
      const ordered=sortBy((_6) => _6[2], _5[1]);
      const p_1=get(ordered, 0);
      const p_2=get(ordered, length(ordered)-1);
      const lastPoint=p_2[4];
      return[p_1[0], p_1[1], (p_1[2]+p_2[2])/2>>0, (maxBy((_6) => _6[3], ordered))[3], {
        Timestamp:lastPoint.Timestamp, 
        Open:p_1[4].Open, 
        High:(maxBy((_6) => _6[4].High, ordered))[4].High, 
        Low:(minBy((_6) => _6[4].Low, ordered))[4].Low, 
        Close:lastPoint.Close, 
        Volume:sumBy((_6) => _6[4].Volume, ordered), 
        Temporal:lastPoint.Temporal
      }];
    }, groupBy((_5) => {
      const a=maximumVisualPoints-1;
      const b=_5[2]*maximumVisualPoints/length(referenceTimestamps)>>0;
      let _6=Compare(a, b)===-1?a:b;
      return[_5[0], _5[3]>1, _6];
    }, projectedCandleSeries)));
    const projectedLinePoints_1=map((_5) => {
      const trace_1=_5[1];
      const lines=_5[3];
      const m_1=trace_1.Kind;
      let _6=m_1.$==1?map((point_1) =>({
        Timestamp:point_1.Timestamp, 
        Value:point_1.Volume, 
        Temporal:point_1.Temporal
      }), _5[2]):m_1.$==2?lines:m_1.$==3?lines:[];
      let _7=projectedLinePoints(referenceTimestamps, _6);
      return[_5[0], trace_1, _7];
    }, preparedTraces);
    const linePoints=map((_5) => {
      let _6;
      const points=_5[2];
      if(length(referenceTimestamps)<=maximumVisualPoints||length(points)<=maximumVisualPoints)_6=points;
      else {
        const a=1;
        const b=maximumVisualPoints/2>>0;
        const bucketCount=Compare(a, b)===1?a:b;
        _6=sortBy((t) => t[0], collect((_7) => {
          const bucket=_7[1];
          const minimum=minBy((_8) => _8[1].Value, bucket);
          const maximum=maxBy((_8) => _8[1].Value, bucket);
          return minimum[0]===maximum[0]?[minimum]:sortBy((t) => t[0], [minimum, maximum]);
        }, groupBy((_7) => {
          const a_1=bucketCount-1;
          const b_1=_7[0]*bucketCount/length(referenceTimestamps)>>0;
          return Compare(a_1, b_1)===-1?a_1:b_1;
        }, points)));
      }
      return[_5[0], _5[1], _6];
    }, projectedLinePoints_1);
    const markerPlacements=assignAggregateMarkerLanes(collect((_5) => {
      const trace_1=_5[1];
      if(!Equals(trace_1.Kind, {$:4}))return[];
        ...
        if(m_1!=null&&m_1.$==1){
          ...
          const o_1=o==null?null:Some(markerPlacementsPrepared(trace_1, o.$0, currentData, referenceTimestamps));
          return o_1==null?[]:o_1.$0;
        }
        else return[];
      }
    }, preparedTraces));
    const p=paddedRange(0, 1, ofSeq(delay(() => append_2(collect((_5) => {
      const point_1=_5[4];
      return[point_1.Low, point_1.High];
    }, projectedCandleSeries), delay(() => collect((_5) => {
      const values=map((_6) => _6[1].Value, _5[2]);
      return Equals(_5[1].Kind, {$:3})?[0].concat(values):values;
    }, projectedLinePoints_1))))));
    const readers=map((_5) => {
      const traceIndex=_5[0];
      const trace_1=_5[1];
      ...
      switch(m_1.$==1?0:m_1.$==2?1:m_1.$==3?1:m_1.$==4?2:0){
        case 0:
          ...
          return[(index_1) => {
            const o_1=tryItem(index_1, values);
            const o_2=o_1==null?null:o_1.$0;
            return o_2==null?null:Some(candleCursorPointValue(label, trace_1.Kind, o_2.$0));
          }, (index_1) => {
            const o_1=tryItem(index_1, values);
            const o_2=o_1==null?null:o_1.$0;
            if(o_2==null)return null;
              ...
              const point_1=o_2.$0;
              let _8={Timestamp:point_1.Timestamp, Value:Equals(trace_1.Kind, {$:1})?fixedText(point_1.Volume):"O "+fixedText(point_1.Open)+" H "+fixedText(point_1.High)+" L "+fixedText(point_1.Low)+" C "+fixedText(point_1.Close)+" V "+fixedText(point_1.Volume)};
              return Some(_8);
            }
          }];
        case 1:
          ...
          const o=tryFind((_8) => _8[0]===traceIndex, projectedLinePoints_1);
          if(o==null)_6=null;
          else {
            o.$0[0];
            o.$0[1];
            const _7=o.$0[2];
            for(let i_2=0, _8=_7.length-1;i_2<=_8;i_2++){
              const f_1=get(_7, i_2);
              const index=f_1[0];
              if(index>=0&&index<length(values_1))set(values_1, index, Some(f_1[1]));
            }
            _6=void 0;
          }
          return[(index_1) => {
            const o_1=tryItem(index_1, values_1);
            const o_2=o_1==null?null:o_1.$0;
            ...
            if(o_2==null)return null;
              ...
              const point_1=o_2.$0;
              let _9={Timestamp:point_1.Timestamp, Value:fixedText(point_1.Value)};
              return Some(_9);
            }
          }];
        case 2:
    ...
    switch(m.$==3?0:m.$==1?0:m.$==2?1:2){
      case 0:
        ...
        return concat_1(" ", map((_2) => {
          ...
          let _4=Compare(a_1, b_1)===1?a_1:b_1;
          return rectanglePath(slot*(_2[0]+0.18), _3, barWidth, _4);
        }, points));
      case 1:
        return concat_1(" ", mapi((_2, _3) =>(_2===0?"M":"L")+" "+fixedText(_3[0])+" "+fixedText(_3[1]), map((_2) =>[xAt(_2[0]), normalize(low, high, top, plotHeight, _2[1].Value)], points)));
      case 2:
        return"";
    }
  };
  ...
  const markerLayer=Doc.EmbedView(Map2((_2, _3) =>(((_4) => {
    ...
    return(viewportWidth) => {
      ...
      const a_1=30;
      const a_2=320;
      let _5=Compare(a_2, viewportWidth)===1?a_2:viewportWidth;
      const b=10000/_5;
      const b_1=Compare(a_1, b)===-1?a_1:b;
      const fontSize=Compare(a, b_1)===1?a:b_1;
      const shapeLaneStep=size+2;
      const labelLaneStep=fontSize+2;
      const labelCandidates=choose((placement) => {
        ...
        const e=height-half;
        const e_1=placement.Marker.Anchor.$==1?anchorY+4+half+placement.Lane*shapeLaneStep:anchorY-4-half-placement.Lane*shapeLaneStep;
        const e_2=Compare(half, e_1)===1?half:e_1;
        const baseY=Compare(e, e_2)===-1?e:e_2;
      ...
      return svgElement("g", [Attr.Create("data-testid", "ta-marker-layer-"+rowId), Attr.Create("data-marker-count", String(length(placements)))], ofSeq_1(delay(() => append_2(map_2((placement) => {
        ...
        switch(m.$==3?1:m.$==4?2:m.$==0?3:m.$==1?3:0){
          case 0:
            return svgElement("circle", append_1(common, ofArray([svgAttr("cx", fixedText(x)), svgAttr("cy", fixedText(y)), svgAttr("r", fixedText(half_1))])), [title]);
          case 1:
            return svgElement("rect", append_1(common, ofArray([svgAttr("x", fixedText(x-half_1)), svgAttr("y", fixedText(y-half_1)), svgAttr("width", fixedText(size_1)), svgAttr("height", fixedText(size_1))])), [title]);
          case 2:
            return svgElement("polygon", append_1(common, ofArray([svgAttr("points", String(fixedText(x))+","+String(fixedText(y-half_1))+" "+String(fixedText(x+half_1))+","+String(fixedText(y))+" "+String(fixedText(x))+","+String(fixedText(y+half_1))+" "+String(fixedText(x-half_1))+","+String(fixedText(y)))])), [title]);
          case 3:
            const o=markerTrianglePoints(placement.Marker.Shape, x, y, half_1);
            let _6=o==null?[]:o.$0;
            let _7=map((_12) => fixedText(_12[0])+","+fixedText(_12[1]), _6);
            let _8=concat_1(" ", _7);
            let _9=[svgAttr("points", _8)];
            let _10=ofArray(_9);
  ...
  const candlePathStates=map((_2) => {
    ...
    return[traceIndex, _2[1], map(_c_3.Create_1, candlePaths(traceIndex, [initialGeometry[0], initialGeometry[1], initialGeometry[2], initialGeometry[3], initialGeometry[4], initialGeometry[5], initialGeometry[6], initialGeometry[7]]))];
  }, distinctBy((t) => t[0], map((_2) =>[_2[0], _2[1]], initialGeometry[1])));
  const lineVisualStates=map((_2) => {
    const traceIndex=_2[0];
    const trace=_2[1];
    return[traceIndex, trace, _c_3.Create_1(linePath(traceIndex, trace, initialGeometry)), _c_3.Create_1(lineLastValue(traceIndex, initialGeometry))];
  }, initialLinePoints);
  observedPreparedData=preparedData;
  Sink((currentData) => {
    if(!(currentData===observedPreparedData)){
      ...
      for(let i=0, _3=candlePathStates.length-1;i<=_3;i++)((() => {
        ...
        const nextPaths=candlePaths(f_1[0], [geometry[0], geometry[1], geometry[2], geometry[3], geometry[4], geometry[5], geometry[6], geometry[7]]);
      ...
      for(let i_1=0, _4=lineVisualStates.length-1;i_1<=_4;i_1++){
  ...
  return[svgElement("svg", [svgAttr("viewBox", "0 0 1000 "+fixedText(height)), svgAttr("preserveAspectRatio", "none"), svgAttr("role", "img"), svgAttr("aria-label", "Composite TA row "+rowId), Attr.Create("data-testid", svgTestId), Attr.Create("data-point-count", String(length(referenceTimestamps))), Attr.Create("style", "display:block; width:100%; height:"+fixedText(height)+"px; background:#fbfcfe;"), Handler("mousemove", (element_2) =>(event) => {
    ...
    const p=find((_2) => _2[0]===traceIndex, lineVisualStates);
    const path=p[2].View;
    const lastValue=p[3].View;
    const m_1=trace.Kind;
    switch(m_1.$==3?0:m_1.$==1?0:m_1.$==2?1:2){
      case 0:
        return[svgElement("path", [Attr.Create("data-testid", "ta-trace-"+rowId+"-"+trace.TraceId), Dynamic_1("d", path), Dynamic_1("data-last-value", lastValue), svgAttr("fill", traceColor), svgAttr("fill-opacity", "0.62")], [])];
      case 1:
        return[svgElement("path", [Attr.Create("data-testid", "ta-trace-"+rowId+"-"+trace.TraceId), Dynamic_1("d", path), Dynamic_1("data-last-value", lastValue), svgAttr("fill", "none"), svgAttr("stroke", traceColor), svgAttr("stroke-width", fixedText(trace.Width)), svgAttr("stroke-linejoin", "round"), svgAttr("stroke-linecap", "round")], [])];
      case 2:
        return[];
    }
...
function timeAxis(testId, rowId, timestamps){
  return Doc.EmbedView(Map((width) => {
    const labels=adaptiveTimeLabels(92, width, timestamps);
    return Doc.Element("div", [Attr.Create("data-testid", testId), Attr.Create("data-time-axis-row-id", rowId), Attr.Create("data-time-axis-tick-count", String(length(labels))), Attr.Create("style", "position:relative; min-width:0; height:18px; padding:0 1px; overflow:hidden;")], ofSeq_1(delay(() => collect_1((position) => {
      ...
      return[Doc.Element("span", [Attr.Create("data-time-axis-event-time", label), Attr.Create("style", "position:absolute; left:"+fixedText(left)+"%; transform:"+transform+"; max-width:92px; color:#708198; font-size:10px; line-height:16px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;")], [Doc.TextNode(compactTimestamp(label))])];
...
function New_56(PreviousNodes, Top){
...
function kindErrors(limits_1, field_1, depth, kind){
  return ofSeq_1(delay(() => append_2(depth>limits_1.MaxSchemaDepth?[error_1("limit-schema-depth", field_1, "Schema depth exceeds hard limit "+String(limits_1.MaxSchemaDepth)+".")]:[], delay(() => {
    if(kind.$==1)return rangeErrors(field_1, kind.$0, kind.$1);
      ...
      return append_2(length(values)===0?[error_1("choice-required", field_1, "Choice editor requires at least one choice.")]:[], delay(() => append_2(length(values)>limits_1.MaxChoicesPerField?[error_1("limit-editor-choices", field_1, "Choices exceed hard limit "+String(limits_1.MaxChoicesPerField)+".")]:[], delay(() => append_2(duplicateKey(field_1, (choice) => choice.Key, values), delay(() => collect_1((m) => {
        const index=m[0];
        const choice=m[1];
      ...
      return append_2(length(values_1)===0?[error_1("scale-required", field_1, "Scale editor requires at least one allowed key.")]:[], delay(() => append_2(duplicateKey(field_1, (x) => x, values_1), delay(() => collect_1((m) => identifier(String(field_1)+".scaleKeys["+String(m[0])+"]", m[1]), indexed(values_1))))));
    }
    else if(kind.$==6){
...
function prepareDataIncremental(previous, data){
  const axisUpdates=choose((_1) => updateAxis(previous, _1[1]), ofSeq(ToSeq(data)));
  ...
  return{
    ...
    ResolvedSeries:OfArray(choose((_1) => {
      ...
      if(value.$==4){
        ...
        return _6!=null&&_6.$==1&&(_6.$0.$==4&&(_7!=null&&_7.$==1&&(_2=[_6.$0.$0, _7.$0],true)))?Some([dataRef, updateInlineSeries(_2[0], _2[1], rawPoints)]):Some([dataRef, map((item) => {
        ...
        if(m==null)return null;
          ...
          if(m_1!=null&&m_1.$==1&&(m_1.$0.Revision===axisRevision&&(_3=m_1.$0,true))){
            const _8=previous.RawData.TryFind(dataRef);
            const _9=previous.ResolvedSeries.TryFind(dataRef);
            if(_8!=null&&_8.$==1&&(_9!=null&&_9.$==1&&(_4=[_9.$0, _8.$0],true))){
              const m_2=tryTemporalSeriesRaw(_4[1]);
              if(m_2!=null&&m_2.$==1&&(m_2.$0,m_2.$0[0]==axisRef&&(_5=[m_2.$0[0], m_2.$0[2]],true))){
                const o=axisChanges.TryFind(axisRef);
                let _10=o==null?null:o.$0;
                let _11=updateTemporalSeries(_5[1], _4[0], rawPoints_1, _3.Points, _10);
                let _12=[dataRef, _11];
                return Some(_12);
              }
              else return Some([dataRef, resolveTemporalPoints(_3.Points, rawPoints_1)]);
            }
            else return Some([dataRef, resolveTemporalPoints(_3.Points, rawPoints_1)]);
          }
          else return Some([dataRef, []]);
        }
      }
    }, ofSeq(ToSeq(data))))
  };
...
function tryAdjacentCoverageRange(direction, maximumBasePoints, document, prepared){
  ...
  if(o_1==null)return null;
    ...
    if(o_2==null)return null;
      ...
      const query=queryDraft(document.DefaultView);
      const _2=tryHead(timeline);
      const _3=tryLast(timeline);
      switch(direction.$==1?_3!=null&&_3.$==1?1:2:_2!=null&&_2.$==1?0:2){
        case 0:
          o=Some([query.FromUtc, _2.$0]);
          break;
        case 1:
          const m=tryBasePointIntervalEndPrepared(baseRow, prepared, _3.$0);
          o=m!=null&&m.$==1?Some([m.$0, query.ToUtcExclusive]):null;
          break;
        case 2:
          o=null;
          break;
      ...
      if(o==null)return null;
        ...
        const _4=o.$0[0];
        const _5=o.$0[1];
        const _6=tryUtcTimestamp(_4);
        const _7=tryUtcTimestamp(_5);
        if(_6!=null&&_6.$==1&&(_7!=null&&_7.$==1&&(Compare(_6.$0, _7.$0)<0&&(_1=[_7.$0, _6.$0],true)))){
          const a=1;
          let _8=Compare(a, maximumBasePoints)===1?a:maximumBasePoints;
          let _9={
            BaseRowId:baseRowId, 
            StartEventTimeUtc:_4, 
            EndEventTimeExclusiveUtc:_5, 
            MaximumBasePoints:_8
          };
          return Some(_9);
        }
        else return null;
...
function fullTimestamp(value){
  const o=timestampParts(value);
  return o==null?null:Some(o.$0[0]+" "+o.$0[1]);
}
function timestampParts(value){
  return IsNullOrWhiteSpace(value)||value.length<19||value[4]!=="-"||value[7]!=="-"||value[10]!=="T"&&value[10]!==" "||value[13]!==":"||value[16]!==":"?null:Some([Substring(value, 0, 10), Substring(value, 11, 8)]);
}
...
function candleSeriesForTracePreparedSampled(maximumCount, trace, prepared){
  ...
  if(_1!=null&&_1.$==1){
    const values=candleSeriesForTracePrepared(trace, prepared);
    return maximumCount<=0||length(values)===0?[]:length(values)<=maximumCount?values:maximumCount===1?[get(values, length(values)-1)]:ofSeq(delay(() => collect_1((sampleIndex) =>[get(values, toInt(Math.round(sampleIndex*(length(values)-1)/(maximumCount-1))))], range(0, maximumCount-1))));
  }
  else return candleSeriesFromResolved(sampleResolvedEvenly(maximumCount, resolvedSeriesPrepared(trace.DataRef, prepared)));
}
...
function referenceTimelineForDocument(document, data){
  const m=document.BaseRowId;
  if(m==null)return referenceTimeline(document.Rows, data);
  else {
    const baseRowId=m.$0;
    const o=tryFind((row) => row.Visible&&row.RowId==baseRowId, document.Rows);
    const o_1=o==null?null:Some(rowTimeline(o.$0, data));
    return o_1==null?[]:o_1.$0;
  }
}
...
function tryUtcTimestamp(value){
  ...
  if(!digitsValid||!fractionValid)return null;
    ...
    const number=(start, count) => Parse(Substring(candidate, start, count));
    const month=number(5, 2);
    const day=number(8, 2);
    const hour=number(11, 2);
    const minute=number(14, 2);
    const second=number(17, 2);
    const year=number(0, 4);
...
function candleSeriesForTracePrepared(trace, prepared){
  ...
  if(m!=null&&m.$==1){
    const refs=m.$0;
    const valuesByTimestamp=(dataRef) => OfArray(map((point) =>[point.Timestamp, point], lineSeriesPrepared(dataRef, prepared)));
...
function sampleResolvedEvenly(maximumCount, values){
  return maximumCount<=0||length(values)===0?[]:length(values)<=maximumCount?values:maximumCount===1?[get(values, length(values)-1)]:ofSeq(delay(() => collect_1((sampleIndex) =>[get(values, toInt(Math.round(sampleIndex*(length(values)-1)/(maximumCount-1))))], range(0, maximumCount-1))));
}
...
function markerLabelGeometry(width, height, fontSize, markerX, markerY, label){
  if(IsNullOrWhiteSpace(label))return null;
    ...
    const bounded=trimmed.length<=48?trimmed:Substring(trimmed, 0, 45)+"...";
    const a=width-4;
    const a_1=24;
    const b=bounded.length*fontSize*0.62;
    const b_1=Compare(a_1, b)===1?a_1:b;
    const estimatedWidth=Compare(a, b_1)===-1?a:b_1;
    const gap=8;
    const edge=2;
    const p=markerX+gap+estimatedWidth<=width-edge?[markerX+gap, "start"]:markerX-gap-estimatedWidth>=edge?[markerX-gap, "end"]:[edge, "start"];
    const a_2=8;
    const a_3=height-8;
    const b_2=Compare(a_3, markerY)===-1?a_3:markerY;
    let _1=Compare(a_2, b_2)===1?a_2:b_2;
    let _2={
      Text:bounded, 
      X:p[0], 
...
function markerLabelLaneY(height, lineStep, anchor, baseY, lane){
  const edge=8;
  const direction=Equals(anchor, {$:0})?-1:1;
  const offset=lane*lineStep;
  const preferred=baseY+direction*offset;
  if(preferred>=edge&&preferred<=height-edge)return preferred;
  else {
    const a=height-edge;
    const b=baseY-direction*offset;
...
function markerLabelCollisionLanes(padding, candidates){
  const ordered=sortBy((_1) =>[_1[1], _1[0]], mapi((_1, _2) => {
    const geometry=_2[1];
    const p=geometry.TextAnchor=="end"?[geometry.X-geometry.EstimatedWidth, geometry.X]:[geometry.X, geometry.X+geometry.EstimatedWidth];
    return[_1, p[0], p[1]];
  }, candidates));
  function firstAvailable(left, laneEnds, lane){
    while(true)
      {
        if(lane>=length(laneEnds)||left>get(laneEnds, lane)+padding)return lane;
        else lane=lane+1;
      }
  }
  function assign(index, laneEnds, assigned){
    while(true)
      {
        if(index>=length(ordered))return assigned;
        else {
          const p=get(ordered, index);
          const right=p[2];
          const lane=firstAvailable(p[1], laneEnds, 0);
          const revisedLaneEnds=lane>=length(laneEnds)?laneEnds.concat([right]):(((m) =>(a) => mapi((_1, _2) =>(m(_1))(_2), a))(((lane_1, right_1) =>(current) =>(value) => current===lane_1?right_1:value)(lane, right)))(laneEnds);
          index=index+1;
          laneEnds=revisedLaneEnds;
          assigned=assigned.Add_1(p[0], lane);
        }
      }
  }
...
let _c_5=Lazy((_i) => class $StartupCode_Renderer {
  ...
  static {
    this.axisViewportWidth=_c_3.Create_1(1440);
...
function Parse_1(s, min_1, max_2, overflowMsg){
  const x=+s;
  if(x!==x-x%1)throw new FormatException("New_1", "Input string was not in a correct format.");
  else if(x<min_1||x>max_2)throw new OverflowException("New_1", overflowMsg);
  else return x;
}
...
function collectUnsafeValue(errors_1, field_1, value){
  if(value.$==3){
    ...
    if(StartsWith(normalized, "javascript:"))errors_1.push(error_1("script-forbidden", field_1, "Script URLs are forbidden."));
    else StartsWith(normalized, "http://")||StartsWith(normalized, "https://")?errors_1.push(error_1("url-forbidden", field_1, "Arbitrary URLs are forbidden in the shared runtime contract.")):void 0;
  }
  else if(value.$==4){
    const values=value.$0;
      ...
      while(e.MoveNext())
        {
          ...
          const key=a[0];
          if(unsafeKey(key))errors_1.push(error_1("unsafe-key", field_1, "Unsafe option key `"+key+"` is forbidden."));
          collectUnsafeValue(errors_1, field_1+"."+key, a[1]);
        }
    }
    finally {
      const _1=e;
...
function New_63(IsCancellationRequested, Registrations){
...
function Create_1(f){
  return New_64(false, f, forceLazy);
}
function forceLazy(){
  const v=this.v();
  this.c=true;
