...
let defaultPcslRoot =
    Path.Combine(__SOURCE_DIRECTORY__, ".pcsl", "poc.dynamic.gpt.fsx")
...
let parseArguments () =
    let parser = ArgumentParser.Create<CliArguments>(programName = "poc.dynamic.gpt.fsx")
    let args = fsiArgs ()
...
let assertActorDynamicRegistration () =
    let dynamicExtension =
        ...
        |> Option.defaultWith (fun () -> failwith "Expected useDynamicSdui to register pulse-trade-comm-spa-dynamic.")

    require
        (dynamicExtension.AppendPageShapes |> List.exists (fun shape -> shape.Shape = "actor-dynamic"))
        "Expected useDynamicSdui manifest to include actor-dynamic append page shape."

let assertActorDynamicCreateRoundTrip () =
    use client = new HttpClient()

    let requestJson =
        Json.serialize
            {| pageId = "gpt-actor-dynamic-smoke"
               title = "GPT Actor Dynamic Smoke"
               setName = "gpt-actor-dynamic-smoke"
               shape = "actor-dynamic"
               tabId = ""
               tabMode = "new"
               path = ""
               description = "poc.dynamic.gpt.fsx actor-dynamic create smoke" |}

    use requestBody = new StringContent(requestJson, Text.Encoding.UTF8, "application/json")
    let response = client.PostAsync(app.Url + "/pages/api/register-page", requestBody).GetAwaiter().GetResult()
    let body = response.Content.ReadAsStringAsync().GetAwaiter().GetResult()

    require response.IsSuccessStatusCode $"actor-dynamic register-page should succeed. status={response.StatusCode} body={body}"
    require (body.Contains("\"shape\":\"actor-dynamic\"")) $"actor-dynamic register-page should round-trip custom shape. body={body}"
...
try
    assertActorDynamicRegistration ()
    assertActorDynamicCreateRoundTrip ()
