...
4. 支援兩種 target binding：
   ...
   - actor key 綁 DU type + union cases list，由 Argu adapter 轉為 DSL target。
...
| `PulseTrade.Comm.Spa.Dynamic` (PTCS.Dynamic) | SDUI DSL types、Canvas/Form renderer、Form state、Argu adapter、target resolver、client extension registration。 |
...
```text
[ actorAddress; duTypeName; unionCase1; unionCase2; ... ]
```

No `1:duType:` / `2:unionCases:` prefix is part of the new canonical key. PTCS core stores and replays the list. PTCS.Dynamic resolves the second segment by checking registered DSL targets first, then registered DU adapters. If the second segment matches neither registry, Dynamic renderer shows a controlled error and does not silently fallback to textarea.

### D5. Argu adapter emits one form section per union case

Argu adapter output must show all requested union cases concurrently. It must not use a union case dropdown for the primary interaction.
...
Form document
  ...
  Section "PFCFGTC"
    ...
    Send button for "PFCFGTC"
```

### D6. Submit returns complete raw Argu only at the adapter boundary
...
### Actor key bound to DU type + cases

```text
Add target
  -> Dynamic add-key renderer picks actorAddress + duTypeName + cases
  -> PTCS stores [ actorAddress; duTypeName; case1; case2; ... ]
Selected key
  -> Dynamic Argu adapter generates SduiDocument from registered IArgParserTemplate type
  -> FormInput renderer shows every selected case as a visible section
Submit
  -> Argu codec emits raw Argu args
  -> PTCS ActorArguTargetCommand.RawArgu
```
...
| `Server/ArguForm.fs` | Refactor from direct Argu schema to Argu-to-FormDsl adapter。 |
| `Client/ArguFormRenderer.fs` | Rename conceptually to FormInput renderer; renderer reads DSL, not DU schema。 |
...
2. `DYN-T-502` Argu adapter: converts `IArgParserTemplate` DU cases into Form DSL sections, all cases visible at once.
3. `DYN-T-503` target resolver: `[actor; formDslId]` and `[actor; duType; cases...]` both resolve; unknown target shows controlled error.
...
6. `DYN-T-506` browser E2E: actor key bound to DU type + cases list renders every selected case and submits raw Argu through RN durable proxy path.

## Open Questions

1. PTCS core extension query callback first slice 是否只支援 option provider，或同時支援 arbitrary form validation provider？
