...
- `PulseTrade.Comm.Spa.Dynamic 0.1.3-beta45` pairs with PTCS `0.2.5-beta55`. `poc.full.nuget.journal.ACL.fsx -- --no-wait --local-port 18082 --github-port 18081 --cluster-port 18787 --pcsl-root .\.pcsl\verify.acl.beta45` verifies the ACL/Login dual-auth path on non-conflict ports: 81-style GitHub OAuth host, 82-style PTCS.Login host, DamnWZ/AssTerry pages, Echo/PingPong target keys, local login, ACL snapshot, Dynamic bundle, durable ActorArgu echo, PingPong stop filtering, fixed actor name reuse, PTCS beta53 Login session-store package compatibility, PTCS beta54 ACL audit compatibility, and PTCS beta55 WebSocket principal revalidation compatibility. Public 81 health/alignment is now `live81-ptcs-beta55-dynamic-beta45-ws-reauth-202607010129`; authenticated visual proof still needs a fresh browser gate. Playwright MCP evidence from the previous package pair remains: `G:\PulseTrade.fs\log\20260630\ptcs-login-acl.png`, `G:\PulseTrade.fs\log\20260630\ptcs-acl-terry-assterry-final.png`.
