...
| DYN-T-609 | Fixed CSS-pixel strokes／default row height | line/SMA authored width夾1–2 CSS px且default／row resize／viewport commit不變；overview左右visual line固定2 CSS px、`#155f73`、pointer-inert；既有handle為transparent rect且drag仍可用；任何resize前全部chart SVG `<=250px`，manual可超過250，reset/reload回capped default | PARTIAL：immutable Renderer `0.1.70` 48/48、Interactive.Client `0.1.61` 12/12與package verifier PASS；source-equivalent browser驗price default=250、manual=282、reset/reload=250且完整通過。Final identity browser run的功能斷言亦通過，但同機負載造成performance gate失敗，待安靜環境重跑；真SPAA consumer pending。 |
