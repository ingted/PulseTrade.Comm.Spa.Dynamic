...
| DYN-WBS-505 | FormInput renderer v2 | Review | 86 | DYN-T-504 / DYN-T-506 / DYN-T-511 | Renderer 已改以 Dynamic metadata 的 Form DSL document/schema registry 決定 form input；PFCF fixture 覆蓋 text/number/enum/tuple/bool/list raw arg codec。2026-06-26 beta4 新增 backend-resolved FormInput DSL fetch path，`[actor; template; canonicalArgString]` selected key 會 POST 到 Dynamic backend resolver，成功後以回傳 DSL document/schema 渲染 form 並帶入 defaults。Remaining：query/stream option source、validation state 與 PTCS.Host live 視覺 E2E。 |
...
| DYN-WBS-508 | Backend target arg-string parser / Form DSL resolver | Done | 100 | DYN-T-507 / DYN-T-509 / DYN-T-511 | `DynamicArgStringTarget` 已支援 `[ actorAddress; duTypeOrTemplateKey; canonicalArgString ]` parser-backed target、controlled parse failure、parsed root cases、root-section order 與 FormInput DSL `DefaultValues` projection；`DynamicArguResolveEndpoint` 已提供 `/client-extensions/dynamic/argu/resolve-target` JSON handler，Expecto 14/14 passed。 |
...
| DYN-WBS-511 | Arg-string backend DSL package gate | Review | 85 | DYN-T-511 | Package gate：backend `DynamicArguResolveEndpoint` parses `[ actorAddress; duTypeOrTemplateKey; canonicalArgString ]` with registered Argu parser and returns FormInput DSL with aliases/defaults；client append renderer compiles against the backend-resolved DSL fetch path。Remaining：live PTCS.Host browser E2E and RN/echo round trip。 |
| DYN-WBS-512 | Arg-string backend DSL Playwright E2E | Planned | 0 | DYN-T-512 | PTCS.Host live Playwright gate：add target key -> backend parse canonical arg string -> receive/render FormInput DSL with aliases/defaults -> submit -> raw Argu string equals expected PFCF data-range command -> RN/echo reply readback。 |
