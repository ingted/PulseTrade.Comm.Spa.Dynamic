...
| DYN-WBS-404 | Append input renderer integration | Review | 85 | DYN-T-404 | 新增 `Client/ArguFormRenderer.fs`，透過 PTCS `PulseTradeRegisterAppendInputRenderer` 註冊 `dynamic-argu-append-input`；selected Dynamic key 時取代 textarea，並在 E2E 驗證 text/number/enum/tuple/bool/list controls 與 submit。Remaining：renderer throw/fallback focused gate 與 browser geometry。 |
| DYN-WBS-405 | Add-key dialog renderer integration | Review | 85 | DYN-T-405 | 新增 Dynamic add-key renderer，回傳 `[ actorAddress; "1:duType:<type>"; "2:unionCases:<caseA>|<caseB>" ]`，PTCS 端解碼後以 `unionCaseNames` string list 提供 renderer context；E2E 已驗證 reload/readback key list 與 full key history。Remaining：duplicate/invalid key/fallback regression。 |
