...
| DYN-WBS-405 | Add-key dialog renderer integration | Review | 95 | DYN-T-405 | 新增 Dynamic add-key renderer，回傳 `[ actorAddress; "1:duType:<type>"; "2:unionCases:<caseA>|<caseB>" ]`，PTCS 端解碼後以 `unionCaseNames` string list 提供 renderer context；E2E 已驗證 reload/readback key list、full key history、built-in add-key fallback，以及 duplicate target key 重送後 projection/card 維持單一項。Remaining：production split-service 後再補跨 process key registry replay。 |
