...
| DYN-WBS-404 | Append input renderer integration | Review | 92 | DYN-T-404 | 新增 `Client/ArguFormRenderer.fs`，透過 PTCS `PulseTradeRegisterAppendInputRenderer` 註冊 `dynamic-argu-append-input`；selected Dynamic key 時取代 textarea，並在 E2E 驗證 text/number/enum/tuple/bool/list controls、raw preview、submit、append renderer throw fallback 與 desktop/mobile geometry。Remaining：invalid submission focused gate。 |
| DYN-WBS-405 | Add-key dialog renderer integration | Review | 90 | DYN-T-405 | 新增 Dynamic add-key renderer，回傳 `[ actorAddress; "1:duType:<type>"; "2:unionCases:<caseA>|<caseB>" ]`，PTCS 端解碼後以 `unionCaseNames` string list 提供 renderer context；E2E 已驗證 reload/readback key list、full key history 與 built-in add-key fallback。Remaining：duplicate/invalid key focused gate。 |
| DYN-WBS-406 | Cross-project RN proxy E2E | Review | 70 | DYN-T-406 / PTC3-T-067 | `G:\PulseTrade2.fs\Libs\PulseTrade.Comm.Spa\Scripts\verify.dynamicArguFormDurableProxy.playwright.fsx` 已完成 first browser/runtime E2E：Dynamic form -> raw Argu args -> `ActorArguTargetCommand.RawArgu` -> PTC RN DurableProxy low-cognitive path -> legacy echo actor -> `ActorArguTargetReply` -> PTCS full target-key history readback。Remaining：split-service RN.Host / ProcSupervisor / ShardingDelivery restart-redelivery / production provider proof。 |
