...
| `DYN-VFY-009` | `DYN-WBS-521` | First slice passed on 2026-07-02 with `src\poc.full.nuget.journal.ACL2.fsx -- --if-dyna-port --no-wait --demo`, PTCS + Dynamic + `PulseTrade.Comm.Spa.ACL 0.1.0-alpha1` + `PulseTrade.Comm.Spa.Login 0.1.0-alpha1`; remaining final gates are production SQL, browser Playwright, public redeploy, and full client bundle extraction。 |
