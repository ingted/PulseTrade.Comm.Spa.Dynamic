...
| `DYN-VFY-009` | `DYN-WBS-521` | Demo, production-SQL no-wait, browser Playwright, and formal service redeploy slices passed on 2026-07-02 with `src\poc.full.nuget.journal.ACL2.fsx` plus PTC Host verification; latest open-provider slice uses PTCS beta70 + Dynamic beta60 + `PulseTrade.Comm.Spa.ACL 0.1.0-alpha10` + `PulseTrade.Comm.Spa.Login 0.1.0-alpha12` and verifies client-extension manifest/script asset registration, WebSharper runtime assets, active Login renderer, active ACL snapshot observer/capability provider, open Login package-owned local auth routes, public 81 OAuth redirect, loopback 82 SQL local login, and direct extension script markers. Remaining final gate is fallback cleanup。 |
