...
| `DYN-TA-T-061` | `DYN-TA-017C` | 同一Contracts suite驗generic editor schema、stable row identity與correlated action lifecycle；reject/conflict不修改authoritative document。 |
| `DYN-TA-T-062..064` | `DYN-TA-017D..F` | Renderer tests、ColdFar Notebook `dotnet dib` fixture及Playwright MCP real-path matrix；synthetic M12僅作regression。 |
