...
| `RFC-PTCS-DYNAMIC-0003` | Draft / Review | Correct product direction to common SDUI DSL: Canvas and FormInput share document model; Argu/DU is an adapter; PTCS.Host owns demo DU。 |
...
| PTCS | `G:\PulseTrade2.fs\Libs\PulseTrade.Comm.Spa\doc\RFC-PTC-SPA-0008.unified-sdui-target-extension-contract.md` | PTCS companion contract for direct DSL target and DU target key binding, renderer context, and safe option-provider query shell。 |
...
| `DYN-T-503` | `DYN-WBS-504` | Package Expecto verifies `DynamicTargetKey.tryResolve` for `[actor; formDslId]` and `[actor; duType; cases...]`, including controlled failures。 |
...
| `DYN-T-506` | `DYN-WBS-506` | Browser E2E for actor key bound to DU type + cases list using PTCS.Host demo DU。 |
