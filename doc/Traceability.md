...
| `DYN-VFY-009` | `DYN-WBS-521` | Demo, production-SQL no-wait, and browser Playwright slices passed on 2026-07-02 with `src\poc.full.nuget.journal.ACL2.fsx`; latest open-provider slice uses PTCS beta70 + Dynamic beta60 + `PulseTrade.Comm.Spa.ACL 0.1.0-alpha10` + `PulseTrade.Comm.Spa.Login 0.1.0-alpha12` and verifies client-extension manifest/script asset registration, WebSharper runtime assets, active Login renderer, active ACL snapshot observer/capability provider, and open Login package-owned local auth routes. Remaining final gates are public redeploy and fallback cleanup。 |
