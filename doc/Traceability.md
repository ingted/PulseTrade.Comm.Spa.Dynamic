...
| `DYN-TA-T-063..064` | `DYN-TA-017E..F` | ColdFar Notebook `dotnet dib` fixture及Playwright MCP real-path matrix；production相容組為Contracts alpha17 / Renderer alpha39 / Interactive.Client alpha10，全部exact `FSharp.Core [10.1.400]`，SessionHost、MDCQ provider identity/cursor與resource lifecycle仍是Daedalus production gate。 |
| `DYN-TA-T-066..068` | `DYN-TA-017G` | Aster generic runtime hardening：atomic retention/resync已以Contracts alpha17 / Renderer alpha39 / Interactive.Client alpha10交付；BaseRowId shared cursor、VisibleRangeChanged與browser application lifecycle依序待續。 |
