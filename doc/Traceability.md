...
| `DYN-TA-T-063..064` | `DYN-TA-017E..F` | ColdFar Notebook `dotnet dib` fixture及Playwright MCP real-path matrix；current production consumer graph為Contracts alpha19 / Renderer alpha45 / Interactive.Client alpha16，全部exact `FSharp.Core [10.1.400]`，SessionHost、MDCQ provider identity/cursor與resource lifecycle仍是Daedalus production gate。 |
| `DYN-TA-T-066..068` | `DYN-TA-017G` | Aster generic runtime hardening：atomic retention/resync與BaseRowId event-time cursor/VisibleRangeChanged已完成；browser application reconnect/lifecycle待續。 |
