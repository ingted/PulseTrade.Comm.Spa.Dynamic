...
- Current package pair: `PulseTrade.Comm.Spa 0.2.5-beta63` + `PulseTrade.Comm.Spa.Dynamic 0.1.3-beta53`.
...
- `DYN-VFY-004` package tests passed `18/18` against beta53.
- `DYN-VFY-008` current command passed through cross-repo PTC gates using beta63/beta53 packages: `dotnet fsi --exec G:\PulseTrade.fs\Libs\PulseTrade.Comm\scripts\run-ptcs-dynamic-nuget-live-host.fsx -- --no-wait`, plus PTCS `Scripts\verify.aclLoginBrowser.playwright.fsx -- --port 0`. It verifies PTCS.ACL/PTCS.Login dual-auth runtime, Dynamic bundle load, durable ActorArgu echo, PingPong stop filtering, fixed actor name reuse, PTCS beta54 ACL audit compatibility, PTCS beta55 WebSocket principal revalidation compatibility, PTCS beta56 WebSocket proxy cleanup compatibility, PTCS beta57 HTTP ACL canonical resource compatibility, PTCS beta58 TLS-offload same-origin compatibility, and PTCS beta63 protected API fetch credentials.
- Playwright MCP evidence: `G:\PulseTrade.fs\log\20260630\ptcs-81-actors-beta63-snapshot.md`, `G:\PulseTrade.fs\log\20260630\ptcs-81-actors-beta63-console.txt`, `G:\PulseTrade.fs\log\20260630\ptcs-81-assterry-beta63-after.md`, `G:\PulseTrade.fs\log\20260630\ptcs-81-assterry-beta63-console.txt`.
...
| DYN-VFY-008 | ACL/Login dual-auth journal POC script | `dotnet fsi --exec .\src\poc.full.nuget.journal.ACL.fsx -- --no-wait --local-port 18082 --github-port 18081 --cluster-port 18787 --pcsl-root .\.pcsl\verify.acl.beta53` | 直接 `#r` PTCS `0.2.5-beta63`、Dynamic `0.1.3-beta53`、ACL/Login Core packages。預設 production-facing mode 是 81 GitHub OAuth + 82 local PTCS.Login；automation 可覆蓋為 18081/18082。腳本使用 SQL journal + PCSL projection、ActorRegistry `ActorOfRegistered` Echo/PingPong actors、DamnWZ/AssTerry actor-argu pages、Echo/PingPong target keys，並驗證 local login、ACL snapshot、Dynamic bundle、durable ActorArgu probe、PingPong stop filtering、fixed actor reuse、PTCS beta54 ACL audit compatibility、PTCS beta55 WebSocket principal revalidation compatibility、PTCS beta56 WebSocket proxy cleanup compatibility、PTCS beta57 HTTP ACL canonical resource compatibility、PTCS beta58 TLS-offload same-origin compatibility與 PTCS beta63 protected API fetch credentials。Playwright MCP 覆蓋 public 81 `/actors` protected API no-401 proof 與 `/page/assterry` FormInput send/reply。 | 本輪修訂：2026-07-01 beta53-ptcs-fetch-credentials |
...
2026-06-29 起，`src\PulseTrade.Comm.Spa.Dynamic.fsproj` 不再以 ProjectReference 消費 PTCS source，改用 exact `PackageReference Include="PulseTrade.Comm.Spa"`；目前版本為 `[0.2.5-beta63]`。驗證重點：
...
- 新 nupkg nuspec 必須包含 `PulseTrade.Comm.Spa [0.2.5-beta63]` dependency。
