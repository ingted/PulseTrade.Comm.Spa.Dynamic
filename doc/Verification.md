...
| DYN-VFY-007 | Full NuGet journal POC script | `dotnet fsi --exec .\src\poc.full.nuget.journal.fsx -- --no-wait` then `dotnet fsi --exec .\src\poc.full.nuget.journal.fsx -- --clear-pcsl-before-start --no-wait` | 直接 `#r` PTCS `0.2.5-beta43` 與 Dynamic `0.1.3-beta32`，用 SQL Server Akka.Persistence journal 作 canonical event source，PCSL root 只作 projection/cache。腳本以 `pcslRoot` hash 派生 SQL DB 名、固定 default cluster port `9787` 與 stable template key `poc-full-nuget-journal-argu`，並把 UI hub backend 包成 `PcslActorProxyCommSpaPersistenceBackend(remoteWire=true)`，避免 `/pages/api/*` direct append 繞過 journal。第二段 gate 先清同一 PCSL projection root，再以 startup warm-up replay append-page registry/key/value、actor registry、participant registry 與 generic set registry，驗證 `pages=1 actors=1` 可從 SQL journal 重建。 | 本輪新增：2026-06-29 poc-full-nuget-journal |
