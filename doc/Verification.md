...
- Current package pair: `PulseTrade.Comm.Spa 0.2.5-beta58` + `PulseTrade.Comm.Spa.Dynamic 0.1.3-beta48`.
...
| DYN-VFY-008 | ACL/Login dual-auth journal POC script | `dotnet fsi --exec .\src\poc.full.nuget.journal.ACL.fsx -- --no-wait --local-port 18082 --github-port 18081 --cluster-port 18787 --pcsl-root .\.pcsl\verify.acl.beta48` | 直接 `#r` PTCS `0.2.5-beta58`、Dynamic `0.1.3-beta48`、ACL/Login Core packages。預設 production-facing mode 是 81 GitHub OAuth + 82 local PTCS.Login；automation 可覆蓋為 18081/18082。腳本使用 SQL journal + PCSL projection、ActorRegistry `ActorOfRegistered` Echo/PingPong actors、DamnWZ/AssTerry actor-argu pages、Echo/PingPong target keys，並驗證 local login、ACL snapshot、Dynamic bundle、durable ActorArgu probe、PingPong stop filtering、fixed actor reuse、PTCS beta54 ACL audit compatibility、PTCS beta55 WebSocket principal revalidation compatibility、PTCS beta56 WebSocket proxy cleanup compatibility、PTCS beta57 HTTP ACL canonical resource compatibility與 PTCS beta58 TLS-offload same-origin compatibility。Playwright MCP 覆蓋 public 81 `/page/assterry` FormInput send/reply。 | 本輪修訂：2026-07-01 beta48-ptcs-tls-offload |
...
2026-06-29 起，`src\PulseTrade.Comm.Spa.Dynamic.fsproj` 不再以 ProjectReference 消費 PTCS source，改用 exact `PackageReference Include="PulseTrade.Comm.Spa"`；目前版本為 `[0.2.5-beta58]`。驗證重點：
...
- 新 nupkg nuspec 必須包含 `PulseTrade.Comm.Spa [0.2.5-beta58]` dependency。
