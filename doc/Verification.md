...
- Current package pair: `PulseTrade.Comm.Spa 0.2.5-beta53` + `PulseTrade.Comm.Spa.Dynamic 0.1.3-beta43`.
...
- `DYN-VFY-004` package tests passed `18/18` against beta43.
- `DYN-VFY-008` current command: `dotnet fsi --exec .\src\poc.full.nuget.journal.ACL.fsx -- --no-wait --local-port 18082 --github-port 18081 --cluster-port 18787 --pcsl-root .\.pcsl\verify.acl.beta43`. It verifies PTCS.ACL/PTCS.Login dual-auth runtime, Dynamic bundle load, durable ActorArgu echo, PingPong stop filtering, and fixed actor name reuse.
...
|---|---|---|---|---|
| DYN-VFY-001 | Package build / WebSharper bundle | `dotnet build .\src\PulseTrade.Comm.Spa.Dynamic.fsproj -c Release` | 建置 Dynamic package 與 WebSharper bundle。2026-07-01 beta43 正常 Release build/pack 通過；若 `wsfsc.exe` 回 `MSB6006 -532462766`，先用 diag log 判斷是否為 stale `wsfscservice.exe` 鎖住 generated `src\websharper.log`，停掉服務並刪 generated log 後重試。若確認是路徑長度問題，才使用短 path fallback：`-p:BaseIntermediateOutputPath=C:\ptcsdyn-release\obj\ -p:OutputPath=C:\ptcsdyn-release\bin\`。 | 本輪修訂：2026-07-01 beta43-full-ws-build |
| DYN-VFY-004 | Package tests | `dotnet run --project .\tests\PulseTrade.Comm.Spa.Dynamic.Tests.fsproj -c Release --no-restore` | 在 DYN-VFY-001 full WebSharper build 通過後，跑 Expecto package tests，總計 `18/18` pass。若 test project 觸發不必要 WebSharper rebuild，可暫時加 `-p:WebSharperRunCompiler=false -p:GeneratePackageOnBuild=false` 作測試層 fallback，但 package gate 仍以 DYN-VFY-001 full build 為準。 | 本輪修訂：2026-07-01 beta43-tests |
...
| DYN-VFY-008 | ACL/Login dual-auth journal POC script | `dotnet fsi --exec .\src\poc.full.nuget.journal.ACL.fsx -- --no-wait --local-port 18082 --github-port 18081 --cluster-port 18787 --pcsl-root .\.pcsl\verify.acl.beta43` | 直接 `#r` PTCS `0.2.5-beta53`、Dynamic `0.1.3-beta43`、ACL/Login Core packages。預設 production-facing mode 是 81 GitHub OAuth + 82 local PTCS.Login；automation 可覆蓋為 18081/18082。腳本使用 SQL journal + PCSL projection、ActorRegistry `ActorOfRegistered` Echo/PingPong actors、DamnWZ/AssTerry actor-argu pages、Echo/PingPong target keys，並驗證 local login、ACL snapshot、Dynamic bundle、durable ActorArgu probe、PingPong stop filtering與 fixed actor reuse。Playwright MCP 覆蓋 login visual 與 admin/Terry UI gating。 | 本輪修訂：2026-07-01 beta43-ptcs-login-store-injection |
...
2026-06-29 起，`src\PulseTrade.Comm.Spa.Dynamic.fsproj` 不再以 ProjectReference 消費 PTCS source，改用 exact `PackageReference Include="PulseTrade.Comm.Spa"`；目前版本為 `[0.2.5-beta53]`。驗證重點：
...
- 新 nupkg nuspec 必須包含 `PulseTrade.Comm.Spa [0.2.5-beta53]` dependency。
