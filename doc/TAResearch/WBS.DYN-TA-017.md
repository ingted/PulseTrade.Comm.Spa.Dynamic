...
| DYN-TA-017C | generic editor schema、versioned correlated action wire/lifecycle、stable row identity | T-061 | 100% | Done：`ptcs-dynamic-action.v1` request/result、single pending、timeout/disconnect/correlation fail-closed；Contracts 15/15與Interactive.Client alpha3 bundle package gate通過。 |
...
- Contracts `0.1.0-alpha11`：`TemporalPoint`保存source interval、scale、observed/available frontier、preview/final與projection；`ptcs-dynamic-action.v1`把action result與authoritative runtime frame分流。Interactive.Client `0.1.0-alpha3` package帶可直接serve的single application bundle、minified bundle、Runtime與manifest。
