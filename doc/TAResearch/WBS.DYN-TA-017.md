...
| DYN-TA-017E | ColdFar Notebook adapter / typed chart root | T-063 | 92% | Aster M15以Interactive.Extension exact alpha16及TradeCore.FsStl win139驗證`FsStlSeries<FloatingPoint>`、time-keyed sequence、Deedle Frame、NestedMap與typed `TA_CHART -> FsStlTaView.compile -> ForQuoteSlot -> Decode`；7 rows／12 requests／12 dataRefs／minimum scale 1皆GREEN。Daedalus M13 exact alpha16 fixed-history notebook亦以真QuoteSlot/session通過3,820 committed bars；尚待production final-cell iframe gate。 |
| DYN-TA-017F | real MDCQ DIB + Playwright MCP + release | T-064 | 80% | alpha16 M15真SPAA與M13 fixed-history `.dib`均GREEN：M15為2 frames、7 rows、4 axes、axis points=`3820/763/127/63`；M13為3,820 committed bars、1/5/30/60K、76 TA series，stderr為空。Playwright已驗桌機shared hover、hide/show與console 0；390x844確認canvas可縮放，但SPAA top form固定七欄會溢出。OPEN_END gate已定位realtime start仍固定2026-07-07而觸發1K 4,000-bar cap；另待per-scale warm-up contract、M13 final-cell iframe與bootstrap transport-accept ACK。 |
