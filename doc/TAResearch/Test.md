...
| DYN-TA-T-063 | REQ-041 | DIB integration | dedicated chart root auto-display，不攔截Expression/FloatingPoint/list formatter；`ApplyTemplate`由Daedalus controller authoritative prepare/swap/release；consumer可在`FSharp.Core 10.1.400` exact graph restore。 | Partial：Aster packages為Contracts alpha19 / Renderer alpha45 / Interactive.Client alpha19且nuspec鎖定`FSharp.Core [10.1.400]`；Daedalus real controller/DIB gate待續。 |
