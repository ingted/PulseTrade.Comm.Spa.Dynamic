...
| DYN-TA-T-063 | REQ-041 | DIB integration | dedicated chart root auto-display，不攔截Expression/FloatingPoint/list formatter；`ApplyTemplate`由Daedalus controller authoritative prepare/swap/release；consumer可在`FSharp.Core 10.1.400` exact graph restore。 | Partial：M15以Interactive.Extension exact alpha14通過`FsStlSeries<FloatingPoint>`、顯式sequence、Deedle Frame、NestedMap、1440K UTC date-only與1380K reject；temporary normalizer已移除。Daedalus typed `TA_CHART`與auto-display尚待發布及iframe gate。 |
| DYN-TA-T-064 | REQ-040..042 | Production E2E | 新版MDCQ real source + `dotnet dib` + Playwright MCP；history/live reconnect/resync、console、geometry與package manifest。 | Blocked：M15已走current SPAA真route且曾取得frames，但fresh provider仍出現`actor-resolve-failed`；OPEN_END/ACK、typed chart、iframe與Playwright尚未全綠，不以fixture替代。 |
