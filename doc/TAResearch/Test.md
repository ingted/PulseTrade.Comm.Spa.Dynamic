...
| DYN-TA-T-063 | REQ-041 | DIB integration | dedicated chart root auto-display，不攔截Expression/FloatingPoint/list formatter；`ApplyTemplate`由Daedalus controller authoritative prepare/swap/release；consumer可在`FSharp.Core 10.1.400` exact graph restore。 | Partial：M15以Interactive.Extension exact alpha16及TradeCore.FsStl win139通過`FsStlSeries<FloatingPoint>`、顯式time-key sequence、Deedle Frame、NestedMap、1440K UTC date-only與1380K reject；`FsStlTaView.compile -> ForQuoteSlot -> Decode`驗得7 rows／12 requests／12 dataRefs／minimum scale 1。M13 fixed-history真QuoteSlot/session亦通過3,820 committed bars；final-cell iframe仍待驗。 |
| DYN-TA-T-064 | REQ-040..042 | Production E2E | 新版MDCQ real source + `dotnet dib` + Playwright MCP；history/live reconnect/resync、console、geometry與package manifest。 | Partial：alpha16 M15真SPAA回2 frames／7 rows／4 axes，points=`3820/763/127/63`；M13 fixed-history回3,820 committed bars／四尺度／76 TA series且stderr為空。Playwright桌機shared hover、hide/show、console 0；行動版canvas正常但SPAA top form overflow已定位。OPEN_END因stale 2026-07-07 start超出1K 4,000-bar cap而在iframe前fail；須修bounded start並決定per-scale warm-up contract，再驗bootstrap transport ACK。 |
