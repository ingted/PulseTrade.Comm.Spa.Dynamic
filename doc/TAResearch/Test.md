...
| DYN-TA-T-063 | REQ-041 | DIB integration | dedicated chart root auto-display，不攔截Expression/FloatingPoint/list formatter；`ApplyTemplate`由Daedalus controller authoritative prepare/swap/release；consumer可在`FSharp.Core 10.1.302` exact graph restore。 | Partial：Aster packages為Contracts alpha15 / Renderer alpha37 / Interactive.Client alpha8且nuspec鎖定`FSharp.Core [10.1.302]`；Daedalus real controller/DIB gate待續。 |
