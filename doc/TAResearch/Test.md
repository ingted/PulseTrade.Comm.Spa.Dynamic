...
| DYN-TA-T-063 | REQ-041 | DIB integration | dedicated chart root auto-display，不攔截Expression/FloatingPoint/list formatter；`ApplyTemplate`由Daedalus controller authoritative prepare/swap/release；consumer可在`FSharp.Core 10.1.400` exact graph restore。 | Partial：M15以Interactive.Extension exact alpha15及TradeCore.FsStl win139通過`FsStlSeries<FloatingPoint>`、顯式time-key sequence、Deedle Frame、NestedMap、1440K UTC date-only與1380K reject；`FsStlTaView.compile -> ForQuoteSlot -> Decode`驗得7 rows／12 requests／12 dataRefs／minimum scale 1。Dynamic Renderer/Interactive.Client `0.1.3` package gate通過。M13才擁有真QuoteSlot/session與final-cell auto-display，尚待iframe gate。 |
| DYN-TA-T-064 | REQ-040..042 | Production E2E | 新版MDCQ real source + `dotnet dib` + Playwright MCP；history/live reconnect/resync、console、geometry與package manifest。 | Partial：alpha15 M15 fixed-range真MDCQ回2 frames、7 rows、4 axes，axis points=`3820/763/127/63`；dev.69後兩輪alpha14與一輪alpha15未重現0-row／timeout／role exit。既有Playwright顯示1K+SMA、5K DMI、30K MACD、60K Heikin-Ashi，shared hover及DMI hide/show通過；Renderer `0.1.3`另驗authored row label一致。M13 alpha15 final-cell iframe/Playwright與OPEN_END history→live ACK仍待驗。 |
