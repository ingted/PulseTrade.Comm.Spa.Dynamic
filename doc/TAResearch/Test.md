...
| DYN-TA-T-063 | REQ-041 | DIB integration | dedicated chart root auto-display，不攔截Expression/FloatingPoint/list formatter；`ApplyTemplate`由Daedalus controller authoritative prepare/swap/release；consumer可在`FSharp.Core 10.1.400` exact graph restore。 | Partial：M15以Interactive.Extension exact alpha18及TradeCore.FsStl win139通過`FsStlSeries<FloatingPoint>`、顯式time-key sequence、Deedle Frame、NestedMap、1440K UTC date-only與1380K reject；`FsStlTaView.compile -> ForQuoteSlot -> Decode`驗得7 rows／12 requests／12 dataRefs／minimum scale 1。M13 fixed-history真QuoteSlot/session亦通過3,820 committed bars；OPEN_END iframe可建立但尚無warm history。 |
| DYN-TA-T-064 | REQ-040..042 | Production E2E | 新版MDCQ real source + `dotnet dib` + Playwright MCP；history/live reconnect/resync、console、geometry與package manifest。 | Partial：alpha18 M15真SPAA回2 frames／7 rows／4 axes，points=`3820/763/127/63`；M13 fixed-history回3,820 committed bars／四尺度／76 TA series且stderr為空。two-phase bridge focused gates已驗初始ACK ordering、sink reject、timeout、prepare/live cancel與single-use。M13 OPEN_END短窗建立iframe，但initial為`committed=0/previews=4`，180秒等不到1K SMA trace；須由MdcQuote owner定案per-scale/tail contract後再驗。 |
...
| DYN-TA-T-070 | REQ-050 | Capacity/package/browser | 3820 positions x 28 compact series serialized frame低於16MiB；working set上限4000；47-operation live patch單frame原子成功，65 operations原子拒絕並resync；exact package、WebSharper、F# Playwright與Playwright MCP無OOM/console error。 | Partial：deterministic frame與generic BrowserDemo實際3,820×28 state均Pass；Contracts 19/19含typed producer validation、47 pass/65 reject；Renderer 25/25、Interactive lifecycle 4/4、Ptcs.Client 15/15、F# Playwright及Playwright MCP desktop/mobile無OOM/console error。待Daedalus real MDCQ/DIB 3,820×28 gate。 |
...
| DYN-TA-T-075 | REQ-053/054 | Client/WebSocket | cache hit先呈現last-good並恰送一次resume；miss送full；Accepted不改data；authoritative patch/full後READY且single channel/timer。 | Partial：browser accepted-state write/rehydrate與paused local pan/zoom/hover、remote range/cursor suppression已由F# Playwright通過；仍待SPAA lifecycle送唯一resume/full並接authoritative frame。 |
| DYN-TA-T-076 | REQ-051..056 | `.dib`/Playwright E2E | 真FSSTL多尺度FloatingPoint/Frame/map workspace，range revisit與reload命中cache，host full/delta取代，MDCQ request bounded、console 0。 | Blocked：待B-D與Daedalus fingerprint/action host；OPEN_END另待MDCQ per-scale warm-up。 |
