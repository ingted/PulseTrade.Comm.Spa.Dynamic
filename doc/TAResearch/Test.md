...
| DYN-TA-T-005 | REQ-004/005 | Component | all TA row kinds, shared x-axis, separate y-scale, unknown-kind error | DYN-TA-003 | Partial：alpha1 render七種row、shared window與separate y-scale；exact model tests 7/7。crosshair/axis detail與研究級DMI/MACD多線仍待。 |
| DYN-TA-T-006 | REQ-005 | Browser | zoom/pan/crosshair/toggle/visibility send no network；cursor values match visible bars | DYN-TA-003 | Partial：F# Playwright證明pan/zoom/reset-view/row toggle callback count保持0；crosshair/cursor values未完成。 |
| DYN-TA-T-007 | REQ-007 | Browser | instrument/interval/range/Add Row each send one typed action with coherent disabled/in-flight state | DYN-TA-003/004 | Partial：query/Add Row/Reset Canvas各送typed action；Ptcs.Client alpha1已把typed query映射為bounded transient frame。真host ack、disabled/in-flight與Playwright待補。 |
