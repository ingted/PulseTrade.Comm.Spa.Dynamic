...
| DYN-TA-T-111 | Fixed CSS-pixel line／navigator boundary／default row height | Renderer browser/real SPAA | SMA／一般line在viewport及row resize前後保持1–2 CSS px；overview左右visual line固定2 CSS px且與transparent drag target分離；任何resize前全部chart SVG `<=250px`，manual可超過250且reset/reload恢復capped default；既有drag行為與owner interaction效能不回歸。 | PARTIAL：immutable exact local graph `Contracts 0.1.28 / Renderer 0.1.70 / Interactive.Client 0.1.61`；48/48、12/12與package verifier全綠。Source-equivalent browser PASS；final identity browser performance受同機負載阻擋，consumer pending。 |
