...
| DYN-TA-T-063 | REQ-041 | DIB integration | dedicated chart root auto-display，不攔截Expression/FloatingPoint/list formatter；`ApplyTemplate`由Daedalus controller authoritative prepare/swap/release；consumer可在`FSharp.Core 10.1.400` exact graph restore。 | Partial：M15以Interactive.Extension exact alpha14通過`FsStlSeries<FloatingPoint>`、顯式sequence、Deedle Frame、NestedMap、1440K UTC date-only與1380K reject；temporary normalizer已移除。current SPAA已消費typed `TA_CHART`並按宣告產生7 rows；Dynamic Interactive.Client `0.1.3` bundle gate通過，尚待win139/alpha15 exact release及cell chart-root auto-display/iframe gate。 |
| DYN-TA-T-064 | REQ-040..042 | Production E2E | 新版MDCQ real source + `dotnet dib` + Playwright MCP；history/live reconnect/resync、console、geometry與package manifest。 | Partial：fixed-range M15真MDCQ回2 frames、7 rows、4 axes，axis points=`763/3820/127/63`；Playwright顯示1K+SMA、5K DMI、30K MACD、60K Heikin-Ashi，shared hover及DMI hide/show通過。Renderer `0.1.3`另驗authored row label在toolbar/card一致且console 0。repeat request曾造成ES access role exit，MdcQuoteAgent正修root cause；OPEN_END/ACK、repeat generation、iframe及exact release仍待驗。 |
